...
Every Maestro frontend feature, the components/hooks/lib that implement it, and the
...
- One WS multiplexes all session PTYs; xterm + fit/webgl/search/web-links/unicode11
  addons. Preserve Maestro's terminal palette.
...
- Inspector "Browser" tab renders a preview URL (or workspace `index.html`) in an
  Electron BrowserView. Mirrors the `maestro preview` CLI command.
...
- List PRs, view summary, claim, trigger Maestro review, submit, merge, resolve comments.
...
- Detect a legacy Maestro install and offer to import projects/sessions.
