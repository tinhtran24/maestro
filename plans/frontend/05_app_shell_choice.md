...
| `daemon-owner`/`daemon-launch`     | Go: reuse `backend`'s daemon launch/attach directly (in-proc or exec `to daemon`) |
...
| `userData` → `~/.thanos/electron`  | pin app data under `~/.thanos` in Go                  |
...
- **Pros**: single Go toolchain shared with the backend; much smaller binary; can
  ...
  to build/ship); consistent with Thanos's original direction.
...
and lock the renderer↔daemon contract, *then* optionally migrate the shell to
