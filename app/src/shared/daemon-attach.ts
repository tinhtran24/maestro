...
//
// (2) is the defensive backstop for issue #367: a standalone `maestro daemon` may be
...
// The daemon's default bind port (backend/internal/config). MAESTRO_PORT overrides it.
export const DEFAULT_DAEMON_PORT = 3001;
// The `service` field every genuine Maestro daemon stamps on its health payloads.
export const DAEMON_SERVICE_NAME = "maestro-daemon";
...
/**
 * The port a freshly spawned daemon is expected to bind: MAESTRO_PORT when set and
...
export function expectedDaemonPort(env: Record<string, string | undefined>): number {
	const configured = env.MAESTRO_PORT ? Number(env.MAESTRO_PORT) : NaN;
