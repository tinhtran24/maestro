...
describe("expectedDaemonPort", () => {
	it("defaults to 3001 when MAESTRO_PORT is unset or empty", () => {
		expect(expectedDaemonPort({})).toBe(3001);
		expect(expectedDaemonPort({ MAESTRO_PORT: "" })).toBe(3001);
		expect(DEFAULT_DAEMON_PORT).toBe(3001);
	});

	it("honors a valid MAESTRO_PORT override", () => {
		expect(expectedDaemonPort({ MAESTRO_PORT: "3037" })).toBe(3037);
		expect(expectedDaemonPort({ MAESTRO_PORT: "1" })).toBe(1);
		expect(expectedDaemonPort({ MAESTRO_PORT: "65535" })).toBe(65535);
	});

	it("falls back to the default for an out-of-range or non-integer MAESTRO_PORT", () => {
		expect(expectedDaemonPort({ MAESTRO_PORT: "0" })).toBe(3001);
		expect(expectedDaemonPort({ MAESTRO_PORT: "70000" })).toBe(3001);
		expect(expectedDaemonPort({ MAESTRO_PORT: "3001.5" })).toBe(3001);
		expect(expectedDaemonPort({ MAESTRO_PORT: "not-a-number" })).toBe(3001);
		expect(expectedDaemonPort({ MAESTRO_PORT: "-1" })).toBe(3001);
	});
});
...
describe("resolveDaemonFromRunFile", () => {
	...
	it("reports an error (not a spawn) when the daemon is up but not ready", async () => {
		...
		expect(result).toEqual({
			...
			executablePath: undefined,
			workingDirectory: undefined,
			message: "An Maestro daemon is already running, but it is not ready yet.",
			code: "not_ready",
		});
	});
	...
	it("surfaces a foreign-daemon identity error", async () => {
		const result = await resolveDaemonFromRunFile({
			...
			identityError: () => "Another Maestro daemon is already running from /other.",
		});
		expect(result).toMatchObject({
			state: "error",
			pid: 4242,
			port: 3001,
			message: "Another Maestro daemon is already running from /other.",
		});
	});
		...
		const result = await resolveDaemonFromPort({
			...
			identityError: (probe) =>
				probe.executablePath === "/new/to"
					? null
					: `Another Maestro daemon is already running from ${probe.executablePath}.`,
		});
		expect(result).toMatchObject({
			state: "error",
			port: 3001,
			pid: 777,
			message: "Another Maestro daemon is already running from /old/to.",
		});
	});
...
describe("end-to-end against a real daemon server", () => {
	...
	// (non-Maestro) server squatting on the port.
	...
	it("does NOT attach to a foreign (non-Maestro) server squatting on the port", async () => {
	...
	// A foreign Maestro daemon (correct service, wrong binary) serving the port. The
	...
	it("surfaces an identity error for a foreign Maestro binary serving the port (does not silently attach)", async () => {
		...
		expect(result).toMatchObject({
			...
			port,
			pid: 909,
			message: "Another Maestro daemon is already running from /old/build/to.",
		});
	});

	// THE #367 SCENARIO: a standalone `maestro daemon` is serving the port, but the
