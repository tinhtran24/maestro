...
// crash dumps) under the canonical Maestro home at ~/.maestro instead of Electron's macOS
// default ~/Library/Application Support/<name>. Keeps the app's entire footprint
// inside ~/.maestro alongside the daemon's data dir and running.json. sessionData and
// crashDumps derive from userData, so this one override reparents them all.
// Must run before app ready.
app.setPath("userData", path.join(os.homedir(), ".maestro", "electron"));
...
function createWindow(): void {
	...
	if (isDev && process.env.MAESTRO_OPEN_DEVTOOLS === "1") {
...
function runFilePath(): string | null {
	if (process.env.MAESTRO_RUN_FILE) return process.env.MAESTRO_RUN_FILE;
...
function telemetryOverrides(): Record<string, string> {
	return {
		MAESTRO_TELEMETRY_EVENTS: process.env.MAESTRO_TELEMETRY_EVENTS ?? "on",
		MAESTRO_TELEMETRY_REMOTE: process.env.MAESTRO_TELEMETRY_REMOTE ?? "posthog",
		MAESTRO_TELEMETRY_POSTHOG_KEY: process.env.MAESTRO_TELEMETRY_POSTHOG_KEY ?? DEFAULT_POSTHOG_PROJECT_KEY,
		MAESTRO_TELEMETRY_POSTHOG_HOST: process.env.MAESTRO_TELEMETRY_POSTHOG_HOST ?? DEFAULT_POSTHOG_HOST,
	};
}
...
function ensureShellEnv(): Promise<void> {
	...
	if (!shellEnvPromise) {
		shellEnvPromise = resolveShellEnv(process.env, runLoginShell).then((resolved) => {
			...
			if (!resolved) {
				console.error("Maestro: could not read the login-shell environment; falling back to a static PATH floor.");
			}
		});
	}
...
function daemonEnv(): NodeJS.ProcessEnv {
	// MAESTRO_OWNER=app marks this daemon as app-spawned so the app can re-link the
	// supervisor on attach (headless `maestro start` daemons get no MAESTRO_OWNER and stay
	// unlinked, preserving their persistence across app quit).
	const ownerTag = { MAESTRO_OWNER: "app" };
...
function daemonIdentityError(launch: DaemonLaunchSpec, probe: DaemonProbe): string | null {
	if (launch.source === "dev") {
		...
		if (!probe.workingDirectory && !probe.executablePath) {
			return "An older Maestro daemon is already running, but it does not report its checkout identity. Stop it and restart this app.";
		}
		if (!cwdMatches && !executableMatches) {
			const actual = probe.workingDirectory ?? probe.executablePath ?? "an unknown location";
			return `Another Maestro daemon is already running from ${actual}; expected this checkout at ${launch.cwd}. Stop the other daemon before using this checkout.`;
		}
		return null;
	}
	...
	if (launch.source === "bundled") {
		if (!probe.executablePath) {
			return "An older Maestro daemon is already running, but it does not report its binary path. Stop it and restart this app.";
		}
		if (!samePath(probe.executablePath, launch.command)) {
			return `Another Maestro daemon is already running from ${probe.executablePath}; expected ${launch.command}. Stop the other daemon before using this app.`;
		}
	}
	return null;
}
 ...
 * headless `maestro start` daemons stay unlinked so they remain persistent after
 * app quit.
 */
function establishSupervisorLink(): void {
	...
	if (addr) {
		...
		supervisorLink = connectSupervisor(addr, {
			log: (msg) => console.log(`Maestro: ${msg}`),
		});
	} else {
		console.warn("Maestro: supervisor link skipped; run-file path unavailable");
	}
}
...
async function refreshDaemonStatus(): Promise<DaemonStatus> {
	...
	if (existing) {
		...
		setDaemonStatus({
			state: "stopped",
			message: "Maestro daemon is no longer reachable.",
...
async function startDaemonInner(startEpoch: number): Promise<DaemonStatus> {
	...
	if (!launch) {
		setDaemonStatus({
			state: "stopped",
			message: "MAESTRO_DAEMON_COMMAND is not configured; renderer uses loopback REST when available.",
			code: "not_configured",
		});
		return daemonStatus;
	}
	...
	if (existing) {
		...
		// previously spawned). Headless `maestro start` daemons (owner unset) stay unlinked
	...
	// daemon answers. The expected port (MAESTRO_PORT or the default) is exactly the
	// port the Go child would bind and collide on — probing a hardcoded 3001 would
	// miss an MAESTRO_PORT override.
	...
	// intentionally NOT handled: an identity-mismatched but healthy Maestro daemon is
	// already surfaced as an error status upstream by resolveDaemonFromPort (not
	// killed here), and a foreign non-Maestro process holding the port with a dead
	...
	if (launch.source === "bundled" && !existsSync(launch.command)) {
		setDaemonStatus({
			state: "error",
			message: `Bundled Maestro daemon binary was not found at ${launch.command}. Rebuild the desktop package.`,
			code: "binary_missing",
		});
		return daemonStatus;
	}
	...
	// Discover the port the daemon ACTUALLY bound rather than trusting MAESTRO_PORT:
	...
	const reportBoundPort = (port: number) => {
		...
		// establishSupervisorLink + shouldLinkOnAttach); headless `maestro start` daemons
	...
	fallbackTimer = setTimeout(() => {
		...
		setDaemonStatus({
			state: "ready",
			port: process.env.MAESTRO_PORT ? Number(process.env.MAESTRO_PORT) : undefined,
...
function scanRepoValidationReason(
	...
	if (name === "__root__") return "Repository name is reserved by Maestro.";
...
// Resolve the bundle path `maestro start` will later `open` and stat as a usable app.
// On macOS process.execPath is .../Maestro.app/Contents/MacOS/<exe>;
// the thing `maestro start` opens is the enclosing `.app` directory, so walk up three
...
// `maestro start` opens the app with `--installed-via=<value>` so the app can record
...
// Write ~/.maestro/app-state.json so `maestro start`'s resolveApp() can find this bundle
...
async function writeAppStateOnLaunch(): Promise<void> {
	// Reuse the same ~/.maestro resolution as running.json; the marker lives beside it
	...
	const runFile = runFilePath();
	if (!runFile) {
		throw new Error("cannot resolve ~/.maestro run-file path; skipping app-state marker");
	}
	const stateDir = path.dirname(runFile);
