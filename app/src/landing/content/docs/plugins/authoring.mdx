...
```typescript
import type { PluginModule, Notifier } from "@tinhtran/ao-core";
...
```typescript
import type { PluginModule } from "@tinhtran/ao-core";
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@tinhtran/ao-core`:

```typescript
import type { PluginModule, Notifier } from "@tinhtran/ao-core";
import { validateUrl } from "@tinhtran/ao-core";
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@tinhtran/ao-core`.
...
| `ao plugin install <reference>`   | Install a plugin by marketplace ID, package name, or local path. Writes the entry to `thanos.yaml`.                                                                                             |
...
All utilities are exported from `@tinhtran/ao-core`. The source lives in `packages/core/src/index.ts`.
