...
description: Write a custom runtime, agent, workspace, tracker, SCM, notifier, or terminal plugin for Maestro.
---
...
Return `true` when the system has what your plugin needs — a binary on PATH, an environment variable, a running process. Maestro uses this to warn about missing dependencies at startup rather than at first use.

### Default export

```typescript
import type { PluginModule, Notifier } from "@tinhtran/to-core";
...
- **`name`** must match the suffix of your package name. `@acme/to-plugin-notifier-pagerduty` → `name: "pagerduty"`. The registry looks up plugins by `slot:name` key.
...
## Create a plugin with `maestro plugin create`

The fastest way to start is `maestro plugin create`. It runs an interactive prompt (using `@clack/prompts`) and writes a ready-to-build package.
...
```bash
to plugin create ./my-plugins/pagerduty \
  --slot notifier \
  --name "PagerDuty" \
  --description "Route urgent alerts to PagerDuty" \
  --author "Alice" \
  --package-name "to-plugin-notifier-pagerduty"
```
...
| Author              | `Alice`                                                           |
| Package name        | `maestro-plugin-notifier-pagerduty` (pre-filled from slot + name) |
...
├── README.md          # Maestro config snippets for local and npm installs
...
```typescript
import type { PluginModule } from "@tinhtran/to-core";
...
Replace the placeholder `create()` body with a real implementation. Import the slot interface from `@tinhtran/to-core`:

```typescript
import type { PluginModule, Notifier } from "@tinhtran/to-core";
import { validateUrl } from "@tinhtran/to-core";
```
...
plugins:
  - name: pagerduty
    source: registry
    package: "to-plugin-notifier-pagerduty"
    version: "^1.0.0"
```
...
All interfaces are defined in `packages/core/src/types.ts` and exported from `@tinhtran/to-core`.
...
| `getEnvironment`   | Env vars for the process (must include `~/.maestro/bin` in PATH)  | No                   |
...
Maestro resolves plugins through four mechanisms, tried in this order:

### 1. Bundled

Plugins under `packages/plugins/` in the Maestro monorepo are registered automatically. No config is required.

### 2. Registry / npm (`source: registry` or `source: npm`)

An npm package. Maestro installs it into its internal plugin cache on first use and keeps the version pinned in `maestro.yaml`.
...
Use `maestro plugin install to-plugin-notifier-pagerduty` to add this block automatically.

### 3. Local path (`source: local`)

A filesystem path. Useful during development. Path is resolved relative to `maestro.yaml`.
...
Maestro will import `dist/index.js` (falling back to `index.js`) from that directory.
...
projects:
  myapp:
    tracker:
      package: "@acme/to-plugin-tracker-jira"
      version: "^1.0.0"
      projectKey: "MYAPP"
```
...
## `maestro plugin` subcommands

| Command                                | Description                                                                                                                                                                                                 |
| -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maestro plugin list`                  | List plugins from the bundled marketplace catalog. Add `--installed` to show only what's in your config. Filter by slot with `--type <slot>`. Add `--refresh` to pull the latest catalog from the registry. |
| `maestro plugin search <query>`        | Search the bundled catalog by name, package, description, or slot.                                                                                                                                          |
| `maestro plugin create [dir]`          | Scaffold a new plugin package interactively.                                                                                                                                                                |
| `maestro plugin install <reference>`   | Install a plugin by marketplace ID, package name, or local path. Writes the entry to `maestro.yaml`.                                                                                                        |
| `maestro plugin update [reference]`    | Update an installer-managed plugin. Pass `--all` to update everything.                                                                                                                                      |
| `maestro plugin uninstall <reference>` | Remove a plugin from the config.                                                                                                                                                                            |

---

## Core utilities available to plugins

All utilities are exported from `@tinhtran/to-core`. The source lives in `packages/core/src/index.ts`.
...
| `readLastActivityEntry`    | Read the last entry from the Maestro activity JSONL (`{workspace}/.maestro/activity.jsonl`).                       |
...
| Export                         | Purpose                                                                                                                                                                        |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `setupPathWrapperWorkspace`    | Install `~/.maestro/bin/gh` and `~/.maestro/bin/git` PATH wrappers and write `.maestro/AGENTS.md` in the workspace. Required for PATH-wrapper agents (Codex, Aider, OpenCode). |
| `buildAgentPath`               | Prepend `~/.maestro/bin` to a PATH string.                                                                                                                                     |
...
For agents that don't have a native session API (most new agents), skip step 3 and rely entirely on the Maestro activity JSONL written by `recordActivity`. The full specification — including hook patterns, `isProcessRunning` requirements, and the 7 required test cases — is in the "Agent Plugin Implementation Standards" section of `CLAUDE.md` in the project root.
...
<Cards>
	...
	<Card
		...
		description="How plugins are referenced in maestro.yaml."
