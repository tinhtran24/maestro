...
description: Write a custom runtime, agent, workspace, tracker, SCM, notifier, or terminal plugin for Thanos.
---
...
Return `true` when the system has what your plugin needs — a binary on PATH, an environment variable, a running process. Thanos uses this to warn about missing dependencies at startup rather than at first use.
...
## Create a plugin with `to plugin create`

The fastest way to start is `to plugin create`. It runs an interactive prompt (using `@clack/prompts`) and writes a ready-to-build package.
...
| Author              | `Alice`                                                      |
| Package name        | `to-plugin-notifier-pagerduty` (pre-filled from slot + name) |
...
├── README.md          # Thanos config snippets for local and npm installs
...
| `getEnvironment`   | Env vars for the process (must include `~/.thanos/bin` in PATH)       | No                   |
...
Thanos resolves plugins through four mechanisms, tried in this order:

### 1. Bundled

Plugins under `packages/plugins/` in the Thanos monorepo are registered automatically. No config is required.

### 2. Registry / npm (`source: registry` or `source: npm`)

An npm package. Thanos installs it into its internal plugin cache on first use and keeps the version pinned in `thanos.yaml`.
...
Use `to plugin install to-plugin-notifier-pagerduty` to add this block automatically.

### 3. Local path (`source: local`)

A filesystem path. Useful during development. Path is resolved relative to `thanos.yaml`.
...
Thanos will import `dist/index.js` (falling back to `index.js`) from that directory.
...
## `to plugin` subcommands

| Command                           | Description                                                                                                                                                                                                 |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `to plugin list`                  | List plugins from the bundled marketplace catalog. Add `--installed` to show only what's in your config. Filter by slot with `--type <slot>`. Add `--refresh` to pull the latest catalog from the registry. |
| `to plugin search <query>`        | Search the bundled catalog by name, package, description, or slot.                                                                                                                                          |
| `to plugin create [dir]`          | Scaffold a new plugin package interactively.                                                                                                                                                                |
| `to plugin install <reference>`   | Install a plugin by marketplace ID, package name, or local path. Writes the entry to `thanos.yaml`.                                                                                             |
| `to plugin update [reference]`    | Update an installer-managed plugin. Pass `--all` to update everything.                                                                                                                                      |
| `to plugin uninstall <reference>` | Remove a plugin from the config.                                                                                                                                                                            |
...
| `readLastActivityEntry`    | Read the last entry from the Thanos activity JSONL (`{workspace}/.thanos/activity.jsonl`).                                 |
...
| Export                         | Purpose                                                                                                                                                         |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `setupPathWrapperWorkspace`    | Install `~/.thanos/bin/gh` and `~/.thanos/bin/git` PATH wrappers and write `.thanos/AGENTS.md` in the workspace. Required for PATH-wrapper agents (Codex, Aider, OpenCode). |
| `buildAgentPath`               | Prepend `~/.thanos/bin` to a PATH string.                                                                                                                           |
...
For agents that don't have a native session API (most new agents), skip step 3 and rely entirely on the Thanos activity JSONL written by `recordActivity`. The full specification — including hook patterns, `isProcessRunning` requirements, and the 7 required test cases — is in the "Agent Plugin Implementation Standards" section of `CLAUDE.md` in the project root.
...
<Cards>
	...
	<Card
		...
		description="How plugins are referenced in thanos.yaml."
