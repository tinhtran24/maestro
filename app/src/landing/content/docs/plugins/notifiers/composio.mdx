...
The bot token is used once to create a Composio connected account. Thanos writes only the resulting `connectedAccountId`.

Run the Gmail setup when you want Thanos notifications by email:
...
Connect Gmail in Composio first, or choose Gmail in `to setup composio` and generate a connect link from an existing Gmail auth config. Thanos does not create Gmail OAuth/auth configs. The setup command finds an active Gmail connected account for the configured `userId` and writes its `connectedAccountId`. If multiple Gmail accounts are available, pass the one Thanos should use:

```bash
to setup composio-mail --email-to alerts@example.com --connected-account-id ca_...
```

You can also ask Thanos to print a Composio Gmail connect URL:

```bash
to setup composio-mail --email-to alerts@example.com --connect
```

Thanos uses an existing Composio Gmail auth config for that link. If you need a specific custom Gmail auth config, pass it explicitly:

```bash
to setup composio-mail --email-to alerts@example.com --connect --auth-config-id ac_...
```

If Google blocks a Gmail connection, fix the Gmail auth config in Composio, then rerun `to setup composio-mail`.
...
If `mode` is omitted for Discord, Thanos uses `webhook` when `webhookUrl` is present and `bot` otherwise.
