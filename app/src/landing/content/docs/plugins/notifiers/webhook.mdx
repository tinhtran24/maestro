...
Point it at any HTTPS endpoint. Thanos POSTs a JSON event. Use this for PagerDuty-style integrations, custom dashboards, or anything we don't have a dedicated notifier for.
...
paste a different endpoint, go back, or cancel before Thanos sends the setup test.
Before saving, Thanos also asks which notification priorities the webhook should
...
Free-form messages sent via `to send` arrive as:

```json
{
	"type": "message",
	"message": "Hello from Thanos",
	"context": {}
}
```
...
- Restrict inbound traffic to your Thanos host via IP allowlist or VPN.
...
- You're building an internal Thanos dashboard of your own.
