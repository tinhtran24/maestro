...
Point it at any HTTPS endpoint. Maestro POSTs a JSON event. Use this for PagerDuty-style integrations, custom dashboards, or anything we don't have a dedicated notifier for.
...
paste a different endpoint, go back, or cancel before Maestro sends the setup test.
Before saving, Maestro also asks which notification priorities the webhook should
...
notifiers:
  webhook:
    plugin: webhook
    url: https://example.com/to-events
    headers:
...
Free-form messages sent via `maestro send` arrive as:

```json
{
	"type": "message",
	"message": "Hello from Maestro",
	"context": {}
}
```
...
- Restrict inbound traffic to your Maestro host via IP allowlist or VPN.
...
- You're building an internal Maestro dashboard of your own.
