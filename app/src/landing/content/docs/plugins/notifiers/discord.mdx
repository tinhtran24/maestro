...
5. Copy the webhook URL and paste it into Thanos.

The command sends a test message before writing Thanos config.
Before saving, Thanos also asks which notification priorities Discord should receive:
...
| `username`     | optional | `Thanos` | Override webhook default username per message                       |
...
Thanos uses Discord embeds, not plain text. Each event shows:
