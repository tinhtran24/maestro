...
5. Copy the webhook URL and paste it into Maestro.

The command sends a test message before writing Maestro config.
Before saving, Maestro also asks which notification priorities Discord should receive:
...
```bash
to setup discord --status
to setup discord --refresh
```
...
| `username`     | optional | `Maestro` | Override webhook default username per message                       |
...
Maestro uses Discord embeds, not plain text. Each event shows:
...
```bash
to notify test --to discord --template basic
to notify test --to discord --template merge-ready --actions
```
