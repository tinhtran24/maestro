...
Notifiers deliver Thanos events — session stuck, PR opened, review requested, CI failed — to wherever you actually pay attention.
...
urgent-only`, `urgent-action`, or `all`. Thanos does not dedupe across channels, so
...
| Event               | When                                   |
| ------------------- | -------------------------------------- |
| `session.spawned`   | `to spawn` succeeds                    |
