...
Notifiers deliver Maestro events — session stuck, PR opened, review requested, CI failed — to wherever you actually pay attention.
...
urgent-only`, `urgent-action`, or `all`. Maestro does not dedupe across channels, so
route only the priorities each destination should receive.

## Testing

```bash
to notify test --to slack --template basic
to notify test --route urgent
to notify test --all --dry-run
```
...
| Event               | When                                   |
| ------------------- | -------------------------------------- |
| `session.spawned`   | `maestro spawn` succeeds               |
