...
[OpenClaw](https://github.com/composio-dev/openclaw) is a local gateway that routes Thanos events into whatever personal alert channel you've wired up — phone push, email, a local sound, etc. If it's running on your machine, this notifier uses it.

## Setup

Run Thanos's interactive helper:
...
4. Write only `thanos.yaml`; Thanos does not generate the token or write shell-profile exports.
...
| `token`              | —                                    | Remote/manual fallback. Avoid for local setups because it stores the secret in Thanos config |
...
| `healthSummaryPath`  | —                                    | Optional local path where OpenClaw writes a summary Thanos can pick up                       |

## Token precedence

Token is read in this order:

1. `token` in `thanos.yaml` if explicitly configured
2. `hooks.token` from `openclawConfigPath`
3. Env var `OPENCLAW_HOOKS_TOKEN` as a legacy fallback

`to setup openclaw --status` probes the OpenClaw gateway and token. To send a
real Thanos test notification through the configured notifier, run:
