...
[OpenClaw](https://github.com/composio-dev/openclaw) is a local gateway that routes Maestro events into whatever personal alert channel you've wired up — phone push, email, a local sound, etc. If it's running on your machine, this notifier uses it.

## Setup

Run Maestro's interactive helper:
...
4. Write only `maestro.yaml`; Maestro does not generate the token or write shell-profile exports.
5. Let you review and change the URL, OpenClaw config path, or routing preset before saving.

Non-interactive variant:

```bash
to setup openclaw --non-interactive --url http://127.0.0.1:18789/hooks/agent --routing-preset urgent-action
```

Refresh an existing setup or inspect status:

```bash
to setup openclaw --refresh
to setup openclaw --status
```
...
| `token`              | —                                    | Remote/manual fallback. Avoid for local setups because it stores the secret in Maestro config |
...
| `healthSummaryPath`  | —                                    | Optional local path where OpenClaw writes a summary Maestro can pick up                       |

## Token precedence

Token is read in this order:

1. `token` in `maestro.yaml` if explicitly configured
2. `hooks.token` from `openclawConfigPath`
3. Env var `OPENCLAW_HOOKS_TOKEN` as a legacy fallback

`maestro setup openclaw --status` probes the OpenClaw gateway and token. To send a
real Maestro test notification through the configured notifier, run:

```bash
to notify test --to openclaw --template basic
```
