...
5. Pick the channel Thanos should post to, authorize, and copy the webhook URL.

The command sends a test message before writing Thanos config.
Before saving, Thanos also asks which notification priorities Slack should receive:
...
| `username`   | optional | `Thanos` | Requested display name                                                                              |
