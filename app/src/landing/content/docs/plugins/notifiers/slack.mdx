...
5. Pick the channel Maestro should post to, authorize, and copy the webhook URL.

The command sends a test message before writing Maestro config.
Before saving, Maestro also asks which notification priorities Slack should receive:
...
```bash
to setup slack --status
to setup slack --refresh
```
...
| `username`   | optional | `Maestro`       | Requested display name                                                                              |
