...
Thanos normalises these internally — the dashboard and lifecycle state machine don't know which you use.
...
**Verification**: GitLab sends the configured secret as a literal string in `X-Gitlab-Token`. Thanos compares this value directly (no HMAC — unlike GitHub's SHA-256 approach).

Polling is still active as a fallback — webhooks are a latency optimisation, not a hard dependency.

## Automated review authors

Thanos ignores review comments from known bot accounts so they don't block the merge-readiness check. The full list:
...
See [Review loop — bot detection](/docs/guides/review-loop#automated-review-bugbot-detection) for how Thanos uses this list during the review-pending → mergeable transition.
