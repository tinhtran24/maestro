...
The [Cursor Agent CLI](https://cursor.com/docs) brings Cursor's editing model to a terminal. Maestro spawns it per session, wraps `gh`/`git`, and tracks activity via the Maestro activity log.
...
- **Launch:** Maestro runs `agent` inside the worktree.
- **Activity tracking:** Maestro writes a JSONL to `{workspace}/.maestro/activity.jsonl` each poll cycle based on terminal output classification. The dashboard reads from there.
- **PR + git tracking:** PATH wrappers at `~/.maestro/bin/gh` / `~/.maestro/bin/git` intercept calls to record PR metadata.
...
<Accordions>
	<Accordion title="Maestro says Cursor Agent isn't detected">
	...
	<Accordion title="Dashboard never leaves `active`">
		Cursor doesn't emit its own session JSONL. Maestro classifies based on terminal output — if your terminal is being
		captured by another tool, the classification can drift. `maestro doctor` will flag this.
	</Accordion>
</Accordions>
