...
description: Every plugin that ships with Thanos, grouped by slot. Mix and match in your `thanos.yaml`.
---

Thanos has **eight plugin slots**. Only one plugin per slot is active at a time, and every slot has a sensible default — you don't have to configure anything unless you want to.
...
<PluginGrid>
	<PluginCard
		...
		description="Retained notifications inside the Thanos dashboard."
	/>
	<PluginCard
		name="Desktop"
		logo="apple"
...
The fastest path is `to plugin create` — it scaffolds a working starter:
