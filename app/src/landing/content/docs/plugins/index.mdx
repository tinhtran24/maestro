...
description: Every plugin that ships with Maestro, grouped by slot. Mix and match in your `maestro.yaml`.
---

Maestro has **eight plugin slots**. Only one plugin per slot is active at a time, and every slot has a sensible default — you don't have to configure anything unless you want to.
...
<PluginGrid>
	<PluginCard
		...
		description="Retained notifications inside the Maestro dashboard."
	/>
	<PluginCard
		name="Desktop"
		logo="apple"
...
The fastest path is `maestro plugin create` — it scaffolds a working starter:

```bash
to plugin create --slot notifier --name pagerduty
```
