...
The macOS attach experience. When you run `to open`, it opens each session in a new iTerm2 tab (or window, with `--new-window`).
...
- **"open-iterm-tab: command not found".** Reinstall Thanos (`npm install -g @tinhtran/to`) — the helper binary lives in the to package.
