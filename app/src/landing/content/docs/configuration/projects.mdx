---
title: Projects
description: "Configure one Thanos project: agent, runtime, workspace, setup commands, rules, tracker, SCM, and per-role overrides."
---

A project is one repository that Thanos can run agents against. The global registry remembers the project identity; the local `thanos.yaml` controls how sessions behave inside that repo.
...
`agent` selects the coding agent Thanos launches for worker sessions.
...
`workspace` controls how Thanos isolates each session's code.
...
If a `postCreate` command fails, Thanos does not start the agent for that session. Keep these commands deterministic.
...
If both are set, Thanos includes both.
...
Thanos usually infers tracker and SCM from the registered repository. Override them only when needed.
...
Run `to start` from the repository root so Thanos can register the project. If the repo moved, remove and re-add or relink the project instead of editing `storageKey` manually.

**The agent starts without environment variables**  
Add `.env` or the relevant tool config directory to `symlinks`. Thanos does not copy secrets into worktrees by default.
...
Make sure the corresponding CLI or token is authenticated for the plugin you use. Thanos does not store provider tokens in project config.
...
<Cards>
	<Card
		...
		description="Configure what Thanos does when CI, reviews, or agent state changes need attention."
	/>
	<Card
		title="Remote access"
