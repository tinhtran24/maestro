---
title: Reactions
description: Configure what Thanos does when CI fails, reviews arrive, agents get stuck, or PRs become ready.
---

Reactions are Thanos's automatic responses to session events. They decide whether Thanos should message an agent, notify a human, or mark a PR as ready for a merge action.

You do not need to configure reactions to start. Thanos ships with defaults for CI failures, review comments, merge conflicts, stuck agents, input prompts, and completed sessions.

## Common Recipes

### Watch Without Auto-Recovery

Use this while onboarding Thanos to a sensitive repo. You still see notifications, but Thanos stops sending recovery prompts to agents automatically.
...
<Callout type="warning">
	`auto-merge` is currently an intent flag, not a bypass. Thanos does not ignore branch protection, reviews, or failing
	checks.
</Callout>
...
| Field            | Type                                        | Purpose                                                    |
| ---------------- | ------------------------------------------- | ---------------------------------------------------------- |
| `auto`           | `boolean`                                   | Whether Thanos should perform the automated action             |
| `action`         | `send-to-agent` \| `notify` \| `auto-merge` | What Thanos does when the reaction fires                       |
...
1. Thanos sends the configured `ci-failed.message` when the session first enters a failed-CI state.
2. On the next poll, Thanos sends a structured follow-up with the failing check names, statuses, and URLs.
...
Thanos throttles review-comment lookups to avoid hammering GitHub or GitLab. Pending human comments and known automation comments are fetched at most once every two minutes per session.

Force a check when you need one immediately:

```bash
to review-check <session-id>
```
...
<Cards>
	...
	<Card
		...
		description="Send Thanos events to any HTTPS endpoint."
	/>
	<Card
		title="Configuration"
