---
title: Reactions
description: Configure what Maestro does when CI fails, reviews arrive, agents get stuck, or PRs become ready.
---

Reactions are Maestro's automatic responses to session events. They decide whether Maestro should message an agent, notify a human, or mark a PR as ready for a merge action.

You do not need to configure reactions to start. Maestro ships with defaults for CI failures, review comments, merge conflicts, stuck agents, input prompts, and completed sessions.

## Common Recipes

### Watch Without Auto-Recovery

Use this while onboarding Maestro to a sensitive repo. You still see notifications, but Maestro stops sending recovery prompts to agents automatically.
...
<Callout type="warning">
	`auto-merge` is currently an intent flag, not a bypass. Maestro does not ignore branch protection, reviews, or failing
	checks.
</Callout>
...
| Field            | Type                                        | Purpose                                                    |
| ---------------- | ------------------------------------------- | ---------------------------------------------------------- |
| `auto`           | `boolean`                                   | Whether Maestro should perform the automated action        |
| `action`         | `send-to-agent` \| `notify` \| `auto-merge` | What Maestro does when the reaction fires                  |
...
1. Maestro sends the configured `ci-failed.message` when the session first enters a failed-CI state.
2. On the next poll, Maestro sends a structured follow-up with the failing check names, statuses, and URLs.
...
Maestro throttles review-comment lookups to avoid hammering GitHub or GitLab. Pending human comments and known automation comments are fetched at most once every two minutes per session.

Force a check when you need one immediately:

```bash
maestro review-check <session-id>
```
...
<Cards>
	...
	<Card
		...
		description="Send Maestro events to any HTTPS endpoint."
	/>
	<Card
		title="Configuration"
