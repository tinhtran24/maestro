---
title: Architecture
description: How Thanos fits together — plugin slots, session lifecycle, event bus, prompt assembly, and activity detection.
---

Thanos (Thanos) is a Node.js orchestrator that spawns and manages parallel AI coding agents across isolated git worktrees. Every moving part is a plugin; the core provides the state machine, event bus, and prompt assembly that ties them together.

## The 8 Plugin Slots

Each abstraction in Thanos is a named interface defined in `packages/core/src/types.ts`. Seven of the eight slots are pluggable at runtime; the eighth (Lifecycle) is built into core and cannot be replaced.
...
<Callout type="info">
	...
	configure its behaviour (poll interval, reactions, thresholds) through `thanos.yaml` rather than by
...
Loaded from `agentRules` (inline string in `thanos.yaml`) and/or `agentRulesFile` (path to a file, relative to the project root). Both are concatenated when present. If neither is provided, this layer is omitted.
...
- **Role rules** — read-only investigations only; never own a PR; never use `tmux send-keys` directly; always use `to send` / `to spawn` to delegate.
- **Project info** — name, repo, default branch, session prefix, local path, dashboard port.
- **Quick-start commands** — `to status`, `to spawn`, `to batch-spawn`, `to session claim-pr`, `to send`, `to open`.
- **Available `to` commands table** — full reference adapted to whether a repo is configured.
...
| **Thanos activity JSONL**  | Aider, OpenCode, new agents | The agent implements `recordActivity`, which calls `recordTerminalActivity()` → `classifyTerminalActivity()` → `appendActivityEntry()` to write to `{workspacePath}/.thanos/activity.jsonl`. `getActivityState` reads from this file. |
...
When an agent creates a PR or switches a branch, Thanos needs to update the session metadata (e.g. write `pr=https://...` or `branch=feat/INT-123`) so the dashboard and lifecycle manager stay in sync. Two mechanisms exist:
...
Agents without a native hook system (Codex, Aider, OpenCode, custom agents) use `~/.thanos/bin/gh` and `~/.thanos/bin/git` shell wrappers. These wrappers are installed to `~/.thanos/bin/` by `setupPathWrapperWorkspace(workspacePath)` from `packages/core/src/agent-workspace-hooks.ts`. The function also writes session context to `{workspacePath}/.thanos/AGENTS.md` (gitignored — does not touch tracked files).
...
```
~/.thanos/{hash}-observability/processes/{component}-{pid}.json
```
...
```
~/.thanos/{hash}-{projectId}/feedback-reports/*.kv
```
...
```
thanos.yaml ──► Config Loader (Zod) ──► Plugin Registry
                                                          │
                                          ┌───────────────┘
                                          │
                                          ▼
                                    Session Manager ◄─── to spawn / to session
                                          │
                                          ▼
...
<Cards>
	<Card
		...
		description="Configure projects, agents, trackers, and reactions in thanos.yaml."
	/>
	<Card
	...
	<Card
		...
		description="Understand the global registry, local project config, and where Thanos stores runtime data."
	/>
</Cards>
