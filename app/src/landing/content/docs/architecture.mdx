---
title: Architecture
description: How Maestro fits together — plugin slots, session lifecycle, event bus, prompt assembly, and activity detection.
---

Maestro is a Node.js orchestrator that spawns and manages parallel AI coding agents across isolated git worktrees. Every moving part is a plugin; the core provides the state machine, event bus, and prompt assembly that ties them together.

## The 8 Plugin Slots

Each abstraction in Maestro is a named interface defined in `packages/core/src/types.ts`. Seven of the eight slots are pluggable at runtime; the eighth (Lifecycle) is built into core and cannot be replaced.
...
<Callout type="info">
	...
	configure its behaviour (poll interval, reactions, thresholds) through `maestro.yaml` rather than by replacing the
	implementation.
</Callout>
...
Loaded from `agentRules` (inline string in `maestro.yaml`) and/or `agentRulesFile` (path to a file, relative to the project root). Both are concatenated when present. If neither is provided, this layer is omitted.
...
- **Role rules** — read-only investigations only; never own a PR; never use `tmux send-keys` directly; always use `maestro send` / `maestro spawn` to delegate.
- **Project info** — name, repo, default branch, session prefix, local path, dashboard port.
- **Quick-start commands** — `maestro status`, `maestro spawn`, `maestro batch-spawn`, `maestro session claim-pr`, `maestro send`, `maestro open`.
- **Available `maestro` commands table** — full reference adapted to whether a repo is configured.
...
| **Maestro activity JSONL** | Aider, OpenCode, new agents | The agent implements `recordActivity`, which calls `recordTerminalActivity()` → `classifyTerminalActivity()` → `appendActivityEntry()` to write to `{workspacePath}/.maestro/activity.jsonl`. `getActivityState` reads from this file. |
...
When an agent creates a PR or switches a branch, Maestro needs to update the session metadata (e.g. write `pr=https://...` or `branch=feat/INT-123`) so the dashboard and lifecycle manager stay in sync. Two mechanisms exist:
...
Agents without a native hook system (Codex, Aider, OpenCode, custom agents) use `~/.maestro/bin/gh` and `~/.maestro/bin/git` shell wrappers. These wrappers are installed to `~/.maestro/bin/` by `setupPathWrapperWorkspace(workspacePath)` from `packages/core/src/agent-workspace-hooks.ts`. The function also writes session context to `{workspacePath}/.maestro/AGENTS.md` (gitignored — does not touch tracked files).
...
```
~/.maestro/{hash}-observability/processes/{component}-{pid}.json
```
...
```
~/.maestro/{hash}-{projectId}/feedback-reports/*.kv
```
...
```
maestro.yaml ──► Config Loader (Zod) ──► Plugin Registry
                                                          │
                                          ┌───────────────┘
                                          │
                                          ▼
                                    Session Manager ◄─── maestro spawn / maestro session
                                          │
                                          ▼
...
<Cards>
	<Card
		...
		description="Configure projects, agents, trackers, and reactions in maestro.yaml."
	/>
	<Card
	...
	<Card
		...
		description="Understand the global registry, local project config, and where Maestro stores runtime data."
	/>
</Cards>
