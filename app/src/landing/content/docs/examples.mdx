...
All examples live in the [`examples/` directory](https://github.com/ComposioHQ/thanos/tree/main/examples) of the Thanos repository. Pick the one closest to your setup, copy it to your project root as `thanos.yaml`, fill in your repo path and any API keys, and you're ready to spawn agents.

```bash
cp examples/simple-github.yaml thanos.yaml
```
...
- No `defaults:` block needed — Thanos's built-in defaults (`tmux` runtime, `claude-code` agent, `worktree` workspace) apply automatically.
- Replace `owner/my-app` and `~/my-app` with your actual GitHub slug and local checkout path.

[View raw file on GitHub](https://github.com/ComposioHQ/thanos/blob/main/examples/simple-github.yaml)
...
[View raw file on GitHub](https://github.com/ComposioHQ/thanos/blob/main/examples/linear-team.yaml)
...
[View raw file on GitHub](https://github.com/ComposioHQ/thanos/blob/main/examples/multi-project.yaml)
...
- `ci-failed.retries: 3` lets Thanos send the agent back to fix CI up to three times before escalating.
...
[View raw file on GitHub](https://github.com/ComposioHQ/thanos/blob/main/examples/auto-merge.yaml)
...
[View raw file on GitHub](https://github.com/ComposioHQ/thanos/blob/main/examples/codex-integration.yaml)
...
<Cards>
	<Card
		...
		description="Full reference for every field in thanos.yaml."
	/>
	<Card
		title="Plugins"
		href="/docs/plugins"
