---
title: CLI reference
description: Every `to` command, every flag, drawn from the source.
---
...
├── update                      Check for updates and upgrade Thanos
├── init                        (deprecated — use to start)
...
Positional `[project]` — project id, local path, or repo URL. If omitted and no config exists in `cwd`, Thanos creates `thanos.yaml`.

If Thanos is already running, you'll get an interactive prompt to open/restart/spawn-new/quit. Non-TTY callers get info + `process.exit(0)`.
...
| `--all`           | —       | Stop all running Thanos instances on this machine    |
...
<Callout type="warn">
	The old two-arg form `to spawn &lt;project&gt; &lt;issue&gt;` is rejected with an error. Use `-p` or run from inside a
	worktree.
</Callout>

### `to batch-spawn <issues...>`
...
If the build looks stale you'll see a suggestion to run `to dashboard --rebuild`.
...
Runs `to-doctor.sh` for shell-level checks, then TypeScript checks (version freshness, plugin resolution, notifier connectivity). OpenClaw is the only notifier probed without side effects.

### `to update`

> Check for updates and upgrade Thanos to the latest version
...
Behavior depends on how Thanos was installed — git, npm-global, pnpm-global, or unknown (manual instructions printed).

### `to config-help`

> Show config schema and guide for creating `thanos.yaml`
...
### `to session claim-pr <pr> [session]`
...
If `[session]` is omitted, falls back to `THANOS_SESSION_NAME` / `THANOS_SESSION` env vars.

### `to session restore <session>`
...
### `to session remap <session>`

> Re-discover and persist OpenCode session mapping for an Thanos session
...
Notifier setup commands that write Thanos config support `--routing-preset <preset>`.
Valid presets are `urgent-only`, `urgent-action`, and `all`. In interactive
mode, Thanos asks which notification priorities the notifier should receive before
writing `notificationRouting`.

| Command                         | Purpose                                                |
| ------------------------------- | ------------------------------------------------------ |
| `to setup dashboard`            | Configure dashboard notification retention and routing |
| `to setup desktop`              | Configure native desktop notifications                 |
| `to setup webhook`              | Configure a generic HTTP webhook                       |
| `to setup slack`                | Configure a Slack incoming webhook                     |
| `to setup discord`              | Configure a Discord incoming webhook                   |
| `to setup composio`             | Open the interactive Composio setup hub                |
| `to setup composio-slack`       | Configure Slack through Composio                       |
| `to setup composio-discord`     | Configure Discord webhook mode through Composio        |
| `to setup composio-discord-bot` | Configure Discord bot mode through Composio            |
| `to setup composio-mail`        | Configure Gmail through Composio                       |
| `to setup openclaw`             | Configure OpenClaw gateway notifications               |

### `to setup openclaw`

> Connect Thanos notifications to an OpenClaw gateway
...
| `--refresh`                     | —                           | Reuse existing values and rewrite Thanos config                                        |
...
Interactive mode (TTY) uses `@clack/prompts` with review/change steps for the gateway URL, OpenClaw config path, and routing preset. OpenClaw owns the token in `hooks.token`; Thanos does not generate it or write shell-profile exports.
...
### `to plugin search <query>`

> Search the bundled marketplace catalog

No flags.

### `to plugin create [directory]`

> Scaffold a new Thanos plugin package
...
| Variable                            | Purpose                                                                              |
| ----------------------------------- | ------------------------------------------------------------------------------------ |
| `THANOS_DATA_DIR`                       | Override the `~/.thanos` base directory.                                                 |
| `THANOS_LOG_LEVEL`                      | `error` · `warn` · `info` · `debug` · `trace`                                        |
| `THANOS_CONFIG_PATH`                    | Absolute path to a specific `thanos.yaml`. Overrides CWD-based search.   |
| `THANOS_SESSION_ID` · `THANOS_SESSION_NAME` | Auto-set inside spawned sessions; used by `claim-pr` default                         |
| `THANOS_PROJECT_ID`                     | Set by Thanos inside spawned sessions. Used by `to spawn` to resolve the active project. |
