---
title: CLI reference
description: Every `maestro` command, every flag, drawn from the source.
---
...
├── update                      Check for updates and upgrade Maestro
├── init                        (deprecated — use maestro start)
...
Positional `[project]` — project id, local path, or repo URL. If omitted and no config exists in `cwd`, Maestro creates `maestro.yaml`.

If Maestro is already running, you'll get an interactive prompt to open/restart/spawn-new/quit. Non-TTY callers get info + `process.exit(0)`.

### `maestro stop [project]`
...
| `--all`           | —       | Stop all running Maestro instances on this machine |
...
<Callout type="warn">
	The old two-arg form `maestro spawn &lt;project&gt; &lt;issue&gt;` is rejected with an error. Use `-p` or run from
	inside a worktree.
</Callout>

### `maestro batch-spawn <issues...>`
...
If the build looks stale you'll see a suggestion to run `maestro dashboard --rebuild`.
...
Runs `maestro-doctor.sh` for shell-level checks, then TypeScript checks (version freshness, plugin resolution, notifier connectivity). OpenClaw is the only notifier probed without side effects.

### `maestro update`

> Check for updates and upgrade Maestro to the latest version
...
Behavior depends on how Maestro was installed — git, npm-global, pnpm-global, or unknown (manual instructions printed).

### `maestro config-help`

> Show config schema and guide for creating `maestro.yaml`
...
### `maestro session claim-pr <pr> [session]`
...
If `[session]` is omitted, falls back to `MAESTRO_SESSION_NAME` / `MAESTRO_SESSION` env vars.

### `maestro session restore <session>`
...
### `maestro session remap <session>`

> Re-discover and persist OpenCode session mapping for an Maestro session
...
Notifier setup commands that write Maestro config support `--routing-preset <preset>`.
Valid presets are `urgent-only`, `urgent-action`, and `all`. In interactive
mode, Maestro asks which notification priorities the notifier should receive before
writing `notificationRouting`.

| Command                              | Purpose                                                |
| ------------------------------------ | ------------------------------------------------------ |
| `maestro setup dashboard`            | Configure dashboard notification retention and routing |
| `maestro setup desktop`              | Configure native desktop notifications                 |
| `maestro setup webhook`              | Configure a generic HTTP webhook                       |
| `maestro setup slack`                | Configure a Slack incoming webhook                     |
| `maestro setup discord`              | Configure a Discord incoming webhook                   |
| `maestro setup composio`             | Open the interactive Composio setup hub                |
| `maestro setup composio-slack`       | Configure Slack through Composio                       |
| `maestro setup composio-discord`     | Configure Discord webhook mode through Composio        |
| `maestro setup composio-discord-bot` | Configure Discord bot mode through Composio            |
| `maestro setup composio-mail`        | Configure Gmail through Composio                       |
| `maestro setup openclaw`             | Configure OpenClaw gateway notifications               |

### `maestro setup openclaw`

> Connect Maestro notifications to an OpenClaw gateway
...
| `--refresh`                     | —                           | Reuse existing values and rewrite Maestro config                                   |
...
Interactive mode (TTY) uses `@clack/prompts` with review/change steps for the gateway URL, OpenClaw config path, and routing preset. OpenClaw owns the token in `hooks.token`; Maestro does not generate it or write shell-profile exports.
...
### `maestro plugin search <query>`

> Search the bundled marketplace catalog

No flags.

### `maestro plugin create [directory]`

> Scaffold a new Maestro plugin package
...
### `maestro plugin uninstall <reference>`
...
| Variable                                      | Purpose                                                                                        |
| --------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `MAESTRO_DATA_DIR`                            | Override the `~/.maestro` base directory.                                                      |
| `MAESTRO_LOG_LEVEL`                           | `error` · `warn` · `info` · `debug` · `trace`                                                  |
| `MAESTRO_CONFIG_PATH`                         | Absolute path to a specific `maestro.yaml`. Overrides CWD-based search.                        |
| `MAESTRO_SESSION_ID` · `MAESTRO_SESSION_NAME` | Auto-set inside spawned sessions; used by `claim-pr` default                                   |
| `MAESTRO_PROJECT_ID`                          | Set by Maestro inside spawned sessions. Used by `maestro spawn` to resolve the active project. |
