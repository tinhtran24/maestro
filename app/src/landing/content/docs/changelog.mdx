---
title: Changelog
description: Release notes for Thanos, including new features, improvements, and fixes by version.
---

Release notes for Thanos, including new features, improvements, and fixes by version.

This page is maintained from weekly release notes. Run `to --version` to check your installed version.
...
- Hardened the CLI with install-aware `to update`, startup update notifications, doctor version checks, JSON output for `to session ls`, config error handling, version mismatch fixes, and default filtering for terminated sessions.
- Moved terminals and sessions onto a single multiplexed WebSocket connection, reducing connection churn and resource leaks.
- Added prompt-driven spawn: `to spawn` can now create sessions from freeform tasks without requiring a tracker issue.
...
- [Package changelogs on GitHub](https://github.com/ComposioHQ/thanos/tree/main/packages)
- [GitHub Releases](https://github.com/ComposioHQ/thanos/releases)
- [`main` commit history](https://github.com/ComposioHQ/thanos/commits/main)
