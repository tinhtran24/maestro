---
title: Platforms
description: Choose the right Maestro runtime, terminal, and notification setup for macOS, Linux, Windows, and remote hosts.
---

import { Callout } from "fumadocs-ui/components/callout";

Maestro's core workflow is the same on every platform: create an isolated workspace, run an agent, track the PR, react to CI and review events, and show progress in the dashboard.
...
| Container or remote VM | `process` or `tmux` | Slack, Discord, webhook            | Use persistent storage for Maestro data and protect the dashboard with your own auth. |
...
| macOS idle sleep prevention while Maestro runs | Supported |

<Callout type="info" title="Lid-close sleep still wins">
	Maestro can prevent idle sleep on macOS while agents run, but it cannot override hardware lid-close sleep. Use normal
...
If you are running Maestro on a headless Linux host, prefer Slack, Discord, or webhook notifications over desktop notifications.
...
| Capability                                                | Status                                                  |
| --------------------------------------------------------- | ------------------------------------------------------- |
| `maestro start`, `maestro stop`, `maestro dashboard`      | Supported                                               |
| `maestro spawn` and `maestro spawn --prompt`              | Supported through the process runtime                   |
...
| tmux attach commands        | Not available          | Dashboard terminal and Maestro session commands |

Use PowerShell or Git Bash for normal CLI commands. If a command behaves differently because of shell quoting, put longer instructions in a file and send them with `maestro send --file`.

## Containers And Remote Hosts

Maestro can run on a remote machine or inside a container, but the dashboard has no built-in authentication. Put it behind your own access layer before exposing it beyond localhost.
...
- Mount or preserve Maestro's data directory so session metadata survives restarts.
...
Start with the recommended runtime for your OS. Change it only when the default runtime does not match where Maestro is running.
