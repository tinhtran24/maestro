---
title: Installation
description: Install Thanos, authenticate the tools it controls, and verify your first project.
---
...
This page gets your machine ready to run Thanos. By the end, the `to` command should be on your `PATH`, your source-control CLI should be authenticated, and `to doctor` should be able to explain what is ready or missing.

<Callout type="info" title="What Thanos installs">
	The published package is `@tinhtran/to`. It provides the global `to` command and includes the built-in CLI, dashboard,
	and plugin packages.
</Callout>
...
Install these before running Thanos:

| Tool            | Required               | Why Thanos needs it                                                                      |
| --------------- | ---------------------- | ------------------------------------------------------------------------------------ |
| Node.js 20+     | Yes                    | Runs the Thanos CLI, dashboard, and plugins.                                             |
...
<Tabs items={["npm", "pnpm", "yarn", "from source"]}>
	...
	<Tab value="from source">
		```bash git clone https://github.com/ComposioHQ/thanos cd thanos pnpm install pnpm build
		pnpm --filter @tinhtran/to link --global ```
	</Tab>
</Tabs>
...
For GitHub projects, Thanos uses `gh` for issue lookup, PR discovery, reviews, CI checks, and merge readiness.
...
Thanos launches an agent CLI inside each worker session. Start with the one you already use, then add more later if you want per-project or per-role choices.
...
### Start Thanos in a repo

Run `to start` from a repository you want Thanos to manage:
...
If there is no `thanos.yaml`, Thanos creates one. It also starts the dashboard and an orchestrator session for the project.

You can also start from a path or clone from a URL:

```bash
to start ~/code/my-repo
to start https://github.com/your-org/your-repo
```
...
`to doctor` checks the launcher, config, plugin resolution, runtime tools, source-control authentication, notifier setup, and stale temporary files. If it reports a fixable issue, run:
...
<Accordions>
	<Accordion title="`to` is not found after install">
		Your global package bin directory is not on `PATH`. Check your package manager's global bin path, then reopen your
		shell and run `to --version` again.
	</Accordion>
	<Accordion title="`gh` is not authenticated">
		Run `gh auth login`, then `gh auth status`. Thanos cannot read GitHub issues, PRs, reviews, or CI checks until `gh` is
		authenticated.
	</Accordion>
	...
	<Accordion title="No agent runtime is detected">
		Install one supported agent CLI and run it once outside Thanos so it can complete its own sign-in flow.
	</Accordion>
	<Accordion title="Port 3000 is already in use">
		`to start` scans for a free port and prints the final dashboard URL. Use the URL in the command output.
	</Accordion>
	<Accordion title="The dashboard fails after an update">
		Run `to dashboard --rebuild` to clean stale dashboard build artifacts and start again.
	</Accordion>
</Accordions>
...
<Cards>
	...
	<Card
		title="Configuration"
		description="Understand the generated `thanos.yaml`."
