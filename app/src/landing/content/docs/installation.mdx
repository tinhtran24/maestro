...
This page gets your machine ready to run Thanos. By the end, the `ao` command should be on your `PATH`, your source-control CLI should be authenticated, and `to doctor` should be able to explain what is ready or missing.

<Callout type="info" title="What Thanos installs">
	The published package is `@tinhtran/ao`. It provides the global `ao` command and includes the built-in CLI, dashboard,
	and plugin packages.
</Callout>
...
<Tabs items={["npm", "pnpm", "yarn", "from source"]}>
	<Tab value="npm">```bash npm install -g @tinhtran/ao ```</Tab>
	<Tab value="pnpm">```bash pnpm add -g @tinhtran/ao ```</Tab>
	<Tab value="yarn">```bash yarn global add @tinhtran/ao ```</Tab>
	<Tab value="from source">
		```bash git clone https://github.com/ComposioHQ/thanos cd thanos pnpm install pnpm build
		pnpm --filter @tinhtran/ao link --global ```
	</Tab>
</Tabs>
