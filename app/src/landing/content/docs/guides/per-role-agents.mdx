...
The orchestrator is itself a long-running agent session (`<prefix>-orchestrator`) launched with `to start`. It has access to all `ao` CLI commands and your project configuration but is explicitly prohibited from owning a branch or PR.
