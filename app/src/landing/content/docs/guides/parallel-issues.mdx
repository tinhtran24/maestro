...
The whole point of Thanos is running agents in parallel. Each session gets its own git worktree, so agents can't stomp each other's branches.

## Batch spawn

`to batch-spawn` takes one or more issue identifiers:
...
For a quick interactive version, just run `to spawn` a few times — `batch-spawn` adds the duplicate detection.
...
```bash
to spawn 42 --agent claude-code
to spawn 42 --agent codex
```

Thanos creates two distinct sessions. Compare the PRs side by side.

## Isolation guarantees

| What's isolated | How                                                                                                                                                                                                                        |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| File edits      | Each session has its own worktree under `~/.worktrees/{projectId}/{sessionId}/`. Session metadata lives under `~/.thanos/{hash}-{projectId}/sessions/{sessionId}` — the worktree itself is in `~/.worktrees/`. |
| Branch name     | Thanos names the branch after the session ID, avoiding collisions                                                                                                                                                              |
...
to status -p my-repo --json
```

## When you want fewer

- `to session kill <sessionId>` — kill one
- `to session cleanup` — kill everything whose PR merged or issue closed (safe; archives metadata)
- `to session cleanup --dry-run` — preview first

<Callout type="info">Need them all gone? `to stop --all` halts every running Thanos instance on this machine.</Callout>
