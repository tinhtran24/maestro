...
type UseBrowserViewOptions = {
	...
	terminated?: boolean;
	/**
	 * Preview target driven by the daemon (via `maestro preview`, streamed over CDC).
	 * When set, the view navigates here automatically; an empty value clears it.
	 */
	previewUrl?: string;
	/**
	 * Monotonic counter the daemon bumps on every `maestro preview` call, even when
	 * previewUrl is unchanged. The view re-navigates whenever it advances, so a
	 * repeated `maestro preview <same-url>` still refreshes (and CDC replays of an
...
export function useBrowserView({
	...
	// slot into a different panel, and opening the inspector (what `maestro preview`
		...
		(url: string) => {
			if (!hasNativeBrowser) {
				...
				setNavState((current) => ({
					...current,
					url: normalized,
					title: normalized ? "Maestro preview" : "",
					isLoading: false,
				}));
	...
	// this on previewRevision (bumped on every `maestro preview` call); older daemons
