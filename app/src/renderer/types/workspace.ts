...
export type WorkspaceSession = {
	...
	/**
	 * Live preview target set by the daemon (via `maestro preview`) and streamed over
 ...
 * maestro's getAttentionLevel (packages/web/src/lib/types.ts),
...
export function attentionZone(session: WorkspaceSession): AttentionZone {
	switch (session.status) {
		...
		// maestro collapses these into one "action" zone by default.
		case "needs_input":
		case "no_signal":
		case "ci_failed":
		case "changes_requested":
