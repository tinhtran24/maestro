...
function workerPreviewLines(session: WorkspaceSession | undefined, provider: string): string[] {
	if (session?.id === "demo-review-stack") {
		return [
			...
			"frontend/src/renderer/components/SessionInspector.tsx: Browser tab selected after maestro preview",
...
function reviewerPreviewLines(session: WorkspaceSession | undefined): string[] {
	return [
		"$ maestro review submit --session " + (session?.id ?? "demo-session"),
		"Reviewing PR #319: browser preview rail renders inside Maestro",
				...
				{showEmptyState && (
					<div className="absolute inset-0 grid place-items-center bg-terminal font-mono text-[13px]">
						<div className="text-center">
							<div className="text-[var(--term-fg)]">Maestro</div>
