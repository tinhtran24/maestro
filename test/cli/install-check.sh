...
# `to start` is now the desktop-app launcher: it resolves an installed app or
...
# with a clear `to start:` error (an unreachable/404 download on amd64, or an
...
if err="$("$THANOS_BIN" start 2>&1)"; then
