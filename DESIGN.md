...
## ⚠️ Design direction — clone maestro verbatim (SUPERSEDES emdash · 2026-06-10)

By explicit user decision (2026-06-10), the renderer **clones the
maestro web app verbatim** in looks and design. This **supersedes the
"match emdash" direction** documented in _Aesthetic Direction_ and the palette
sections below — where they conflict, **maestro wins**. Do not re-flag
"this doesn't match emdash" in QA/review; flag divergence from **maestro**.

- **Reference (the user's own app):** `~/Projects/maestro/packages/web/src`
...
- **Build with shadcn primitives** where a component fits (`components/ui/*`:
  dropdown-menu, select, card, table, tooltip, …); maestro's own
...
- The one carried-over divergence still holds: the **accent is refined blue**, and
  the **terminal keeps its own palette**. Everything else tracks maestro.
...
resizable`, react-resizable-panels v4 `collapsible` panel + imperative API,
  ...
  width persist. The Maestro reference keeps the rail always visible.
- **Approved divergence (2026-06-12):** the shell topbar spans the full window
  ...
  strip (user-requested). The Maestro reference keeps a full-height sidebar with the
...
- **What this is:** ReverbCode is an Electron desktop app for supervising many parallel
  AI coding-agent sessions, backed by a Go daemon (`backend/`). The `maestro` CLI is the
...
- The **Orchestrator is the human-facing coordinator**: you talk to it; it spawns
  workers (`maestro spawn`), messages them (`maestro send`), tracks progress, and synthesizes
...
> **Superseded (2026-06-10):** see the _Design direction — clone maestro
> verbatim_ banner at the top. The emdash framing below is retained for history; the
> live look tracks maestro (same flat near-black / hairline family, so most
...
- **Right — worker session:** a **PR/CI status pill** that is the action
  ...
  (rename, restart, kill, claim PR — the `maestro session …` commands).
...
(the topbar `+ New worker`, a project row's hover `+`, or `maestro spawn`) open a modal that
...
- A faithful HTML reference of all of the above (both views + topbar + spawn modal,
  ...
  `~/.gstack/projects/tinhtran-maestro/designs/design-system-20260609/`.
...
| 2026-06-09 | Spawn modal mirrors emdash's Create Task                               | Consistency with the reference; mapped to `maestro spawn` params.                                  |
