# `maestro start` Bootstrapper + npm Deprecation: Implementation Spec
...
>
> **This is NOT a new JS launcher package.** The `maestro` binary that npm ships is the
> existing Go cobra CLI (`backend/cmd/maestro`). This effort rewrites one subcommand,
> `maestro start`, to fetch and open the desktop app. Everything else in the CLI is
...
npm `maestro` is the **legacy on-ramp** for users who already have `maestro` on their PATH.
We are deprecating npm as an app-distribution path:

- `npm update` swaps in our **new Go `maestro` binary** (the whole CLI), replacing the
  old one in place. No fresh-install story; the audience is existing users.
- The **`maestro start`** subcommand is rewritten: instead of starting a daemon, it
...
`maestro start` is the one-time bridge that moves a CLI user onto the canonical,
...
| Fact                         | Value                                                         | Source                          |
| ---------------------------- | ------------------------------------------------------------- | ------------------------------- |
| Product / bundle name        | **`Maestro.app`** (spaced)                                    | `frontend/forge.config.ts:9,50` |
| Bundle id                    | `dev.maestro.desktop`                                         | `frontend/forge.config.ts:8`    |
| Executable name              | `maestro`                                                     | `frontend/forge.config.ts`      |
| **Release repo (canonical)** | **`AgentWrapper/maestro`**                                    | per release owner               |
| Forge publisher repo (TODAY) | `tinhtran24/maestro` — **stale, must change to AgentWrapper** | `frontend/forge.config.ts:86`   |
| GitHub release mode          | **`draft: true`**, `prerelease: false`                        | `frontend/forge.config.ts`      |

> `tinhtran24/maestro` was the **temporary** home during the rewrite; the
> code is now ported and releases land on **`AgentWrapper/maestro`**.
> The forge publisher still points at `tinhtran` and must be corrected (task T3).
> The Go **module path** is also `github.com/tinhtran24/maestro`; renaming
...
- **Matrix: `[macos-latest, windows-latest]` only** (`:28`) — no Linux; deb/rpm
  makers configured but never run (upstream issue AgentWrapper/maestro#2191).
- Maker outputs (today): macOS `@electron-forge/maker-zip` → versioned `.zip`
  under `out/make/zip/darwin/<arch>/`; Windows `MakerNSIS` → `Maestro
Setup.exe` (per-user installer); Linux `maker-deb`/`maker-rpm` →
  `maestro-<version>.{deb,rpm}`.
...
- Frontend `frontend/package.json` `version: "0.0.0"`; daemon
  ...
  `go build ./cmd/maestro` with **no `-ldflags`**. No real semver anywhere.
...
### 1.5 `~/.maestro` state and app lifecycle

- Canonical home `~/.maestro` (`backend/internal/config/config.go:296`,
  `frontend/src/shared/daemon-discovery.ts:107`); overrides `MAESTRO_DATA_DIR`/`MAESTRO_RUN_FILE`.
- `userData` pinned to `~/.maestro/electron` (`main.ts:64`, before `whenReady`; CLAUDE.md
  hard rule).
- `~/.maestro/running.json` is written by the **daemon** (`backend/internal/runfile/runfile.go`
  `Write`, atomic temp+rename), read by the app (`daemon-discovery.ts parseRunFile`).
  Only `running.json` exists in `~/.maestro` today; **`app-state.json` does not exist yet**.
- App startup (`main.ts:822` `whenReady`): `registerRendererProtocol()` →
  `createWindow()` → `void startDaemon()` → `initAutoUpdates()`. The app already
  **spawns and owns the daemon** (`startDaemon`, spawns the bundled `maestro daemon`).
...
- The `maestro` binary is `backend/cmd/maestro` (`cmd/maestro/main.go` → `cli.Execute()`); the
  same binary serves as both the CLI and `maestro daemon`. `build-daemon.mjs` builds it
  to `frontend/daemon/to` and bundles it into the desktop app.
- **This repo has no npm-registry publish path for the `maestro` binary** (only
  electron-forge → GitHub Releases; no `NPM_TOKEN`, no publish workflow — research
  confirmed). The old Maestro npm package shipped `maestro` via npm; that delivery mechanism
...
### 1.7 The Go `maestro` CLI surface (already wired)

`backend/cmd/maestro/main.go` → `backend/internal/cli`. Cobra root (`root.go:154-202`)
...
(`doctor.go` is 20KB of health checks; `import.go` imports a legacy Maestro install).
The CLI is a thin client: commands "discover the local daemon, call its loopback
HTTP API, and format output" (`root.go:1-3`).

**Current `maestro start` (`start.go:54-119`):** starts the daemon (spawns `maestro daemon`,
...
1. **Releases land on `AgentWrapper/maestro`.** Fix the forge publisher
   to match; the download URL uses it.
2. **`maestro start` = fetch + open the desktop app.** It no longer starts the daemon;
   the frontend owns the daemon. The current daemon-spawn logic in `start.go` is
   removed.
3. **npm ships the Go `maestro` binary**; existing users update in place. No JS launcher
   package.
4. **Marker = `~/.maestro/app-state.json`**, written only by the app, every launch.
...
7. **Two release targets, never conflated:**
   - **Production:** GitHub `AgentWrapper/maestro`; npm = the real
     package name (legacy `maestro`). Cutting a prod release is a deliberate, gated
     step, never part of the dev/test loop.
   - **Test/dev:** GitHub **`harshitsinghbhandari/maestro`** (the fork);
     npm scope **`@theharshitsingh/to`**. All `maestro start` download/open testing runs
...
- Rewrite the Go **`maestro start`** subcommand: `resolve → fetch → open` the desktop
  app, then print a deprecation notice. (`backend/internal/cli/start.go`.)
- Decide the fate of `maestro start`'s current first-boot legacy import (§6.4).
- **App-side:** write `~/.maestro/app-state.json` every launch (app is sole writer);
  own `moveToApplicationsFolder()` relocation (macOS).
- **Release wiring:** point forge publisher at `AgentWrapper/maestro`,
  add stable version-free asset names, finalize the draft (or Releases-API
  fallback), add Linux to the matrix.
- **npm delivery** of the Go binary (port the old Maestro mechanism; zero install
...
3. **The app is the sole writer of `app-state.json`.** `maestro start` is read-only with
...
4. **The app owns relocation** (`moveToApplicationsFolder()`), and rewrites the
   marker path afterward. `maestro start` never moves the app.
5. **`maestro start` is dumb about versions.** Decision is present-or-absent only; never
...
7. **Stable, version-free release asset names** so `maestro start` uses a constant URL.

---

## 5. The marker contract: `~/.maestro/app-state.json`
...
{
	"schemaVersion": 1,
	"appPath": "/Applications/Maestro.app",
	"version": "0.0.0",
...
| `version`          | app    | `app.getVersion()`. For the tour/migration, NOT for `maestro start` update decisions. |
...
stale/missing marker no matter how the app arrived. `maestro start` only reads it, only
after `stat`-ing the path.

---

## 6. The `maestro start` subcommand (Go) — the heart of this effort
...
(`Deps.CommandOutput`, `Deps.LookPath`, `Deps.Executable`) and the `~/.maestro`
resolution already in `backend/internal/config`.

### 6.2 `resolveApp()` (invariants 2, 5, 6)

1. Read `~/.maestro/app-state.json`; if `appPath` `stat`s as a usable bundle, return it.
2. Else scan known locations per platform (covers website installs / stale marker):
   - macOS: `/Applications/Maestro.app`, `~/Applications/…`
   - Windows: `%LOCALAPPDATA%\Programs\maestro\…`, `C:\Program Files\Maestro\…`
   - Linux: `/opt/Maestro/…`, `~/.local/bin`, `/usr/bin`
...
**build-time overridable**, not hardcoded: default `AgentWrapper/maestro`
(prod), overridden to `harshitsinghbhandari/maestro` for test builds via
...
`maestro start` currently runs `maybeFirstBootImport` (`start.go:84`, imports a legacy
Maestro install before the daemon starts). With the daemon-spawn removed, this must
move. Options (decide §11): (a) the **desktop app** runs the import when it first
boots its daemon; (b) drop it from `maestro start` and rely on the standalone `maestro
import` command (still wired). Recommended: (a), so the on-ramp still migrates
existing data.

### 6.5 Other subcommands / bare `maestro`
...
`~/.maestro/app-state.json`. On first creation, capture `installSource` from the
`--installed-via` arg `maestro start` passes (else `website`/`github`/`unknown`).
...
- **Publisher repo is overridable** (`forge.config.ts:86`): default prod
  `AgentWrapper/maestro`, but read from an env var (e.g.
  `MAESTRO_RELEASE_REPO`) so a fork build publishes to
  `harshitsinghbhandari/maestro`. The dev loop publishes a draft+finalize
...
- **Stable asset names:** add a release-workflow step renaming each maker output to
  space-free names (`maestro-darwin-arm64.zip`,
  `maestro-win32-x64.exe`, the Linux artifact per §11) before upload.
...
auto-update; users self-update by re-running `maestro start` or downloading from the
website.
...
| 2   | `npm i -g @theharshitsingh/to --ignore-scripts` (v12 sim)             | Install succeeds; `maestro` runs; `maestro start` works (binary delivered via optionalDeps, not a script).                                            |
| 3   | Fresh macOS `maestro start`                                           | Fetches `.zip`, `ditto`-unpacks, opens `Maestro.app`; app relocates to `/Applications`; `~/.maestro/app-state.json` records the `/Applications` path. |
| 4   | Website install first, then `maestro start`                           | Known-location scan finds it; opens; no second copy fetched.                                                                                          |
| 5   | App trashed (marker stale), then `maestro start`                      | Marker `stat` misses → scan misses → re-fetch.                                                                                                        |
| 6   | App relocated by the app                                              | Marker path rewritten; next `maestro start` opens the right path; no orphan.                                                                          |
| 7   | Installed-but-old app, `maestro start`                                | Opens it and exits; does NOT fetch a newer one.                                                                                                       |
| 8   | Windows `maestro start`                                               | Downloads NSIS `.exe`, runs installer, resolves + opens installed exe.                                                                                |
| 9   | Linux `maestro start`                                                 | Fetches chosen artifact and launches.                                                                                                                 |
| 10  | `maestro stop`/`maestro status`/`maestro spawn` after `maestro start` | Work against the app-owned daemon (CLI is a client).                                                                                                  |
| 11  | Existing CLI user runs `npm update` then `maestro start`              | New binary in place; `maestro start` no longer starts a daemon, it opens the app; their `maestro import` data migrates (per §6.4).                    |

> `maestro start` opens the app through the calling shell's enriched env, so a green
> `maestro start` proves nothing about the Dock-launch path. Test the Dock path
> separately.
...
1. **npm delivery mechanism** for the Go binary: per-platform `optionalDependencies`
   packages (recommended, zero-install-script) vs porting whatever the old Maestro
   package did. Test scope is **`@theharshitsingh/to`**; the **prod package name**
   (the legacy `maestro` users already have) still needs confirming, plus an `NPM_TOKEN`
   - publish workflow for each.
2. **Legacy first-boot import** (§6.4): move into the desktop app, or drop from
   `maestro start` and rely on `maestro import`?
...
## 12. Task breakdown (for Maestro execution, dependency-ordered)

**Batch 1 — wiring (parallel):**

- **T1. Rewrite `maestro start` core (Go).** Replace `start.go` daemon-spawn with
  ...
  `maestro start` opens it and writes nothing; with it absent, it fetches+opens.
...
@theharshitsingh/to --ignore-scripts` yields a working `maestro`.
- **T3. Release repo + asset wiring (override-driven).** Make the forge publisher
  repo + the `maestro start` download repo build-time overridable (§6.3, §8); add the
  ...
  `harshitsinghbhandari/maestro` release whose
...
- **T4. App-side marker + relocation** (`main.ts whenReady`, §7.1). Check: a
  packaged launch writes/updates `~/.maestro/app-state.json` with the real bundle path.
- **T5. macOS `maestro start` end-to-end against the FORK release** (needs T3): build the
  test `maestro` with `cli.releaseRepo=harshitsinghbhandari/maestro`, install
  it from `@theharshitsingh/to`, run `maestro start`. Check: acceptance #3–#7 on a mac,
