...
- SCM observer (`internal/observe/scm`) wired into the daemon: GitHub provider,
  ...
  ([#75](https://github.com/tinhtran24/maestro/issues/75),
  [#108](https://github.com/tinhtran24/maestro/issues/108),
  [#109](https://github.com/tinhtran24/maestro/issues/109)).
...
- Agent adapter platform under `internal/adapters/agent/` (23 adapters) with a
  registry and `maestro hooks` activity dispatch.
...
- **Tracker lane**: GitHub tracker adapter exists, but there is no daemon
  ...
  nothing at runtime ([#112](https://github.com/tinhtran24/maestro/issues/112)).
- **Full raw PR/tracker fact surfacing**: the SCM observer writes facts and the
  ...
  ([#110](https://github.com/tinhtran24/maestro/issues/110)) and in
  `maestro session get` ([#111](https://github.com/tinhtran24/maestro/issues/111))
  is still open.
- **CLI parity for PR/review actions**: merge, resolve-comments, and review are
  HTTP-only (frontend-driven); there are no `maestro pr` / `maestro review` commands.

Tracking milestone:
[`rewrite`](https://github.com/tinhtran24/maestro/milestone/1).
