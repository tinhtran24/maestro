...
type Agent interface {
	// GetConfigSpec describes the agent-specific config keys Maestro can
	// expose to users in the Maestro config.
	GetConfigSpec(ctx context.Context) (ConfigSpec, error)

	// GetLaunchCommand builds the argv Maestro should run to start this agent.
	GetLaunchCommand(ctx context.Context, cfg LaunchConfig) (cmd []string, err error)

	// GetPromptDeliveryStrategy tells Maestro whether the prompt is included in
	...
	// GetAgentHooks installs or merges Maestro hooks into the agent's
...
// ConfigSpec describes the agent-specific config keys Maestro can expose to users.
...
// ConfigFieldType is the primitive value kind Maestro expects for a field.
...
// SessionRef identifies an Maestro session whose agent-owned metadata may be read.
...
// PromptDeliveryStrategy describes how Maestro should deliver the initial prompt.
