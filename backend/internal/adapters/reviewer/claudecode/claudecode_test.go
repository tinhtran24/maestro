...
func TestReviewCommandLaunchesReadOnlyOffBypass(t *testing.T) {
	...
	if !contains(agent.got.AllowedTools, "Read") || !contains(agent.got.AllowedTools, "Bash(maestro review submit:*)") {
...
func TestAllowlistCoversPromptRequiredPipedCommands(t *testing.T) {
	...
	for _, cmd := range []string{
		...
		"printf '%s' '{ \"reviews\": [] }' | maestro review submit --session sess-1 --reviews -",
	} {
