...
// token in `maestro hooks <agent> <event>` onto the function that interprets that
// agent's hook callbacks as an Maestro activity state.
//
// The hidden `maestro hooks` CLI command dispatches a live callback through it. Every
// adapter that installs `maestro hooks <tok>` callbacks must have a deriver
...
// DeriveFunc maps a native agent hook event and its raw stdin payload onto an Maestro
...
// Derivers maps the agent token in `maestro hooks <agent> <event>` to its deriver.
...
// a registered deriver here means its adapter installs `maestro hooks <harness>`
...
// harness can ever report (idle). Harness names and `maestro hooks` agent tokens are
