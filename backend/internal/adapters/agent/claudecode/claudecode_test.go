...
func TestGetAgentHooksInstallsClaudeHooks(t *testing.T) {
	...
	if m := matcherForCommand(config.Hooks["SessionStart"], "maestro hooks claude-code session-start"); m == nil || *m != "startup" {
		t.Fatalf("SessionStart matcher = %v, want startup", m)
	}
	if m := matcherForCommand(config.Hooks["UserPromptSubmit"], "maestro hooks claude-code user-prompt-submit"); m != nil {
	...
	if m := matcherForCommand(config.Hooks["Notification"], "maestro hooks claude-code notification"); m != nil {
		t.Fatalf("Notification matcher = %v, want none", m)
	}
	if m := matcherForCommand(config.Hooks["SessionEnd"], "maestro hooks claude-code session-end"); m != nil {
...
func TestUninstallHooksRemovesClaudeHooks(t *testing.T) {
	...
	// which held only Maestro hooks, are removed entirely.
	for _, spec := range claudeManagedHooks {
...
// the given command — used to prove no duplicate Maestro hooks.
...
func TestGetRestoreCommandFallsBackToDerivedUUID(t *testing.T) {
	...
	// from the Maestro session id, the explicit fallback.
	cmd, ok, err := (&Plugin{resolvedBinary: "claude"}).GetRestoreCommand(context.Background(), ports.RestoreConfig{
		Permissions: ports.PermissionModeBypassPermissions,
...
func TestEnsureWorkspaceTrustedCreatesEntry(t *testing.T) {
	...
	work := "/Users/me/.maestro/worktrees/01ABC"
