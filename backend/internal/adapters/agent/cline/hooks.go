...
//
// Maestro installs one wrapper script per managed event. Each script forwards the
// hook payload to `maestro hooks cline <subcommand>` and emits the no-op
// continuation result Cline expects. Scripts carry a marker line so install is
// idempotent and uninstall recognizes Maestro-owned scripts without an embedded
...
const (
	...
	// clineHookCommandPrefix identifies the hook commands Maestro owns. The CLI hook
	// dispatcher routes "maestro hooks cline <subcommand>" to DeriveActivityState.
	clineHookCommandPrefix = "maestro hooks cline "

	// clineHookMarker tags Maestro-generated hook scripts so install/uninstall can
	// distinguish them from user-authored Cline hooks in the same directory.
	clineHookMarker = "# to-managed-cline-hook"
)

// clineHookSpec describes one hook Maestro installs: the native Cline hook event
// (used as the script's filename) and the Maestro sub-command its wrapper forwards
...
type clineHookSpec struct {
	...
	Event string
	// Subcommand is the fixed Maestro hook sub-command name the wrapper invokes.
	Subcommand string
}

// clineManagedHooks is the source of truth for the hooks Maestro installs. The
// native Cline events are mapped onto Maestro's fixed sub-command names so activity
...
// GetAgentHooks installs Maestro's Cline hook scripts into the worktree-local
// `.clinerules/hooks/` directory. Existing user-authored hook scripts are
// preserved, and re-running install simply rewrites Maestro-owned scripts in place.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	// Only scripts Maestro actually wrote go into the workspace .gitignore: a
...
// UninstallHooks removes Maestro's Cline hook scripts from the workspace-local
...
// AreHooksInstalled reports whether any Maestro Cline hook script is present in the
...
// hook payload (JSON on stdin) to the Maestro CLI hook dispatcher and prints the
// no-op continuation result Cline expects ({"cancel": false}). The marker line
// identifies it as Maestro-owned.
func renderClineHookScript(subcommand string) string {
	var b strings.Builder
	b.WriteString("#!/usr/bin/env bash\n")
	b.WriteString(clineHookMarker + "\n")
	// Forward stdin to the Maestro dispatcher; ignore its exit code so a missing/old
	// `maestro` binary can never block Cline's own execution.
