...
//
// Thanos installs one wrapper script per managed event. Each script forwards the
// hook payload to `to hooks cline <subcommand>` and emits the no-op
// continuation result Cline expects. Scripts carry a marker line so install is
// idempotent and uninstall recognizes Thanos-owned scripts without an embedded
...
const (
	...
	// clineHookCommandPrefix identifies the hook commands Thanos owns. The CLI hook
	// dispatcher routes "to hooks cline <subcommand>" to DeriveActivityState.
	clineHookCommandPrefix = "to hooks cline "
...
// (used as the script's filename) and the Thanos sub-command its wrapper forwards
...
type clineHookSpec struct {
	...
	Event string
	// Subcommand is the fixed Thanos hook sub-command name the wrapper invokes.
	Subcommand string
}

// clineManagedHooks is the source of truth for the hooks Thanos installs. The
// native Cline events are mapped onto Thanos's fixed sub-command names so activity
...
// GetAgentHooks installs Thanos's Cline hook scripts into the worktree-local
// `.clinerules/hooks/` directory. Existing user-authored hook scripts are
// preserved, and re-running install simply rewrites Thanos-owned scripts in place.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	// Only scripts Thanos actually wrote go into the workspace .gitignore: a
...
// UninstallHooks removes Thanos's Cline hook scripts from the workspace-local
...
// AreHooksInstalled reports whether any Thanos Cline hook script is present in the
...
// hook payload (JSON on stdin) to the Thanos CLI hook dispatcher and prints the
// no-op continuation result Cline expects ({"cancel": false}). The marker line
// identifies it as Thanos-owned.
