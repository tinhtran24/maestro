...
// plugin runtime, so Maestro bridges it the same two ways it bridges opencode:
...
//     so GetAgentHooks installs an Maestro-owned plugin file (see hooks.go) into
//     .kilocode/plugins/ instead of merging JSON.
//   - Its interactive TUI exposes no permission flag (the --auto flag lives only
//     on `kilo run`, not the default TUI command Maestro launches) and no
//     system-prompt flag. Maestro's graduated permission modes are delivered via the
...
const (
	...
	// `maestro spawn --agent`. It matches domain.HarnessKilocode.
	adapterID = "kilocode"
)
...
// `kilo run`, not on the default TUI command Maestro launches, so passing any
...
// kilocodePermissionConfig maps an Maestro permission mode onto Kilo's permission
