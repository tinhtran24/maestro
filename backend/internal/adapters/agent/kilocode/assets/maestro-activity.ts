// maestro: managed kilocode activity plugin (do not edit)
//
...
// It maps Kilo's native lifecycle events onto Maestro's normalized activity events:
//   session.created                        -> `maestro hooks kilocode session-start`
//   message.updated / message.part.updated  -> `maestro hooks kilocode user-prompt-submit`
//   permission.ask hook                     -> `maestro hooks kilocode permission-request`
//   session.status (status.type == idle)    -> `maestro hooks kilocode stop`
//
// The native session id (and prompt/model where known) is piped to the hook
// command as JSON on stdin, run with cwd set to the worktree so Maestro can correlate
// the Kilo session to its Maestro session. Every invocation is best-effort and must
// never crash the user's Kilo session: a missing `maestro` binary is a guarded no-op
...
export const aoActivity: Plugin = async ({ directory, client }) => {
  // maestro hooks must never be able to hang Kilo: cap each invocation, matching
  ...
  // Wrap in `sh -c` with a guard so a missing `maestro` binary is a silent no-op
  // (exit 0) rather than a per-event error in the user's session.
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v to >/dev/null 2>&1; then exit 0; fi; exec maestro hooks kilocode ${hookName}`]
  }
  ...
  function logHookFailure(hookName: string, detail: string) {
    try {
      void client?.app
        ?.log?.({ body: { service: "maestro-activity", level: "error", message: `hook ${hookName} failed: ${detail}` } })
        ?.catch?.(() => {})
    } catch {
  ...
  //      session-start, so Maestro would see the prompt before the session is
  ...
  //
  // A non-zero exit (the guard makes a missing `maestro` exit 0, so this is a real
  // `maestro hooks` failure) or a spawn exception is logged with its stderr and never
  ...
  // prompt never reaches Maestro and title-from-prompt metadata breaks. Therefore: an
  ...
  return {
    // permission.ask fires when Kilo needs the user to approve a tool call. Maestro
      ...
      try {
        switch (event.type) {
          ...
          case "session.status": {
            ...
            // Maestro's "stop" hook means "the current turn is idle/finished", not
