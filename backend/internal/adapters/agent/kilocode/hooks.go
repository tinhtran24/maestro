...
const (
	...
	// `.kilocode`, and `.opencode` (it is an opencode fork). Thanos writes the
	// branded `.kilocode/plugins/` so the Thanos plugin lands in Kilo's own dir and
	...
	// kilocodePluginFileName is the Thanos-owned plugin file. Thanos fully owns this
	...
	kilocodePluginFileName = "thanos-activity.ts"

	// kilocodePluginSentinel marks the file as Thanos-managed. AreHooksInstalled and
	...
	kilocodePluginSentinel = "thanos: managed kilocode activity plugin"

	// kilocodeHookCommandPrefix identifies the hook commands Thanos owns. The
	// embedded plugin shells `to hooks kilocode <event>`; this prefix is the
	// shared contract with the `to hooks` CLI dispatcher and is asserted by tests
	// so the plugin can't silently drift away from it.
	kilocodeHookCommandPrefix = "to hooks kilocode "
)

// kilocodePluginSource is the Thanos-managed Kilo Code plugin, embedded so it ships
...
// string literal because it is plugin source code, not a data structure Thanos
...
// the plugin wires every one via the `to hooks kilocode <event>` command, and
...
// GetAgentHooks installs Thanos's Kilo Code activity plugin into the worktree-local
...
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	if _, err := os.Stat(pluginPath); err == nil {
		...
		if !managed {
			return fmt.Errorf("kilocode.GetAgentHooks: refusing to overwrite non-Thanos file at %s — move it so Thanos can install its plugin", pluginPath)
		}
...
// UninstallHooks removes Thanos's Kilo Code plugin from the workspace-local
// .kilocode/plugins/ directory. It deletes the file only when it carries the Thanos
...
// AreHooksInstalled reports whether Thanos's Kilo Code plugin is present in the
