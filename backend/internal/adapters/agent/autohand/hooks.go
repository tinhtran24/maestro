...
const (
	...
	// autohandHookCommandPrefix identifies the hook commands Maestro owns, so
	// install skips duplicates and uninstall recognizes Maestro entries by prefix
	// without an embedded template to diff against.
	autohandHookCommandPrefix = "maestro hooks autohand "
	autohandHookTimeout       = 30
)

// autohandManagedHookKeys are the entry keys Maestro owns. On marshal they are
...
// hooks.hooks array. Maestro owns the five typed fields; any other key the user set
// on an entry (matcher, filter, async, ...) is captured in Extra so a rewrite
// preserves fields Maestro does not own instead of silently dropping them.
type autohandHookEntry struct {
	...
	// Extra holds keys Maestro does not manage, captured on unmarshal and written
...
// UnmarshalJSON decodes the entry's typed fields and captures every key Maestro does
...
// MarshalJSON writes Maestro's managed fields merged with any preserved unknown keys
// from Extra. Managed fields win on key collision so Maestro's values stay
// authoritative.
...
// autohandHookSpec describes one hook Maestro installs. Event is Autohand's native
// lifecycle event name; Subcommand is the Maestro hook sub-command appended after the
...
// autohandManagedHooks is the source of truth for the hooks Maestro installs. Each
// native Autohand event is routed to the Maestro sub-command DeriveActivityState
...
// GetAgentHooks installs Maestro's Autohand hooks into the Autohand config's
// hooks.hooks array. Existing user hooks are preserved and duplicate Maestro commands
...
// runtime, so Maestro installs into that config rather than a per-workspace file. The
// AUTOHAND_CONFIG env var, when set, takes precedence so Maestro and the agent agree
// on the target.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	...
	for _, spec := range autohandManagedHooks {
		...
		entries = append(entries, autohandHookEntry{
			Event:       spec.Event,
			Command:     command,
			Description: "Maestro activity hook",
			Enabled:     true,
...
// UninstallHooks removes Maestro's Autohand hooks from the config's hooks.hooks
...
// enables every Autohand hook, not just Maestro's.
...
// AreHooksInstalled reports whether any Maestro Autohand hook is present in the
...
// "hooks" section (preserving keys Maestro doesn't manage such as "enabled"), and the
