...
// GitignoreSentinel marks a workspace .gitignore as Maestro-managed so
...
const GitignoreSentinel = "# managed by maestro: Maestro hook files stay out of git status"

// EnsureWorkspaceGitignore writes a self-ignoring .gitignore into dir covering
// the named Maestro-installed files. Hook files land in fresh session worktrees as
// untracked files, and `git worktree remove` (without --force) refuses on ANY
// untracked file — without this ignore, Maestro's own hook files would make every
// session workspace permanently undeletable. The patterns are anchored to dir
// and name only Maestro's files, so anything else an agent drops in the same
