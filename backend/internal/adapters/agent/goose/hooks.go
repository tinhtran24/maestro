...
const (
	...
	// gooseHookCommandPrefix identifies the hook commands Thanos owns, so install
	// skips duplicates and uninstall recognizes Thanos entries by prefix.
	gooseHookCommandPrefix = "to hooks goose "
	gooseHookTimeout       = 30
)

// gooseManagedHooks is the source of truth for the hooks Thanos installs. Goose
// groups every hook under the nil matcher. Goose has no permission/approval
// lifecycle event yet, so Thanos installs only the session/prompt/stop signals.
...
// GetAgentHooks installs Thanos's Goose hooks, preserving user-defined hooks.
func (p *Plugin) GetAgentHooks(ctx context.Context, cfg ports.WorkspaceHookConfig) error {
	return gooseHooks.Install(ctx, cfg.WorkspacePath)
}

// UninstallHooks removes Thanos's Goose hooks, leaving user-defined hooks untouched.
func (p *Plugin) UninstallHooks(ctx context.Context, workspacePath string) error {
	return gooseHooks.Uninstall(ctx, workspacePath)
}

// AreHooksInstalled reports whether any Thanos Goose hook is present.
