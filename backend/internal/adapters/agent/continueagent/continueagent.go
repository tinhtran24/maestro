...
// agent. Its binary is "cn" (npm package @continuedev/cli) and the Maestro harness /
...
// callbacks through the existing "maestro hooks claude-code <evt>" dispatcher — no
...
// adapterID is the Maestro harness / manifest id. It is the string "continue"
...
//
// Maestro sessions are long-lived terminal sessions, so prompted and promptless
// launches both stay interactive as `cn ...`. Permission flags map Maestro's 4 modes
...
//
// The installed commands are "maestro hooks claude-code <evt>", so the existing CLI
...
// (SessionStart / UserPromptSubmit / Stop / Notification), giving Maestro
...
// appendApprovalFlags maps Maestro's 4 permission modes onto Continue's two boolean
