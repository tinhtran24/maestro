// maestro: managed opencode activity plugin (do not edit)
//
// It maps opencode's native lifecycle events onto Maestro's three normalized
// activity events:
//   session.created                       -> `maestro hooks opencode session-start`
//   message.updated / message.part.updated -> `maestro hooks opencode user-prompt-submit`
//   session.status (status.type == idle)   -> `maestro hooks opencode stop`
//
// The opencode-native session id (and prompt/model where known) is piped to the
// hook command as JSON on stdin, run with cwd set to the worktree so Maestro can
// correlate the opencode session to its Maestro session. Every invocation is
// best-effort and must never crash the user's opencode session: a missing `maestro`
...
export const aoActivity: Plugin = async ({ directory, client }) => {
  // maestro hooks must never be able to hang opencode: cap each invocation, matching
  ...
  function hookCmd(hookName: string): string[] {
    return ["sh", "-c", `if ! command -v to >/dev/null 2>&1; then exit 0; fi; exec maestro hooks opencode ${hookName}`]
  }
