...
// no-update-check, hook-trust bypass, and approval flags, Maestro's session-flag
...
// DoctorLaunchProbes returns argv tails `maestro doctor` runs against the installed
// codex binary to smoke-test the launch surface Maestro's hook delivery depends on.
...
// codex's config with Maestro's `-c` session-flag overrides through the offline
...
func appendHideRateLimitNudgeFlag(cmd *[]string) {
	...
	// In a headless Maestro pane that dialog hangs the session invisibly and
...
func appendHookTrustBypassFlag(cmd *[]string) {
	// Maestro's activity hooks ride the launch command as session-flag config (see
	...
	// interactive hooks review, leaving Maestro without activity signals.
...
func appendApprovalFlags(cmd *[]string, permissions ports.PermissionMode) {
	...
	case ports.PermissionModeDefault:
		// Codex sessions are Maestro-managed and run headlessly inside a terminal
