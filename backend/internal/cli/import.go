...
func newImportCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "import",
		Short: "Import projects from a legacy Maestro install",
		Long: "Import reads the legacy Maestro flat-file store " +
			"(~/.maestro) read-only and ports its projects and per-project " +
	...
	cmd.Flags().StringVar(&opts.from, "from", "", "Legacy Maestro root to read (default ~/.maestro)")
...
func (c *commandContext) runImport(cmd *cobra.Command, opts importOptions) error {
	...
	if live, err := runfile.CheckStale(cfg.RunFilePath); err != nil {
		...
		return usageError{fmt.Errorf("the Maestro daemon is running (pid %d); stop it first with `maestro stop` before importing", live.PID)}
	}
	...
	if !legacyimport.HasLegacyData(root) {
		_, err := fmt.Fprintf(cmd.OutOrStdout(), "No legacy Maestro projects found at %s. Nothing to import.\n", root)
		return err
	}
