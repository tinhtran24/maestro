...
//
//	go build -ldflags "-X github.com/tinhtran/thanos/backend/internal/cli.releaseRepo=harshitsinghbhandari/agent-orchestrator" ./cmd/to
//
...
func (c *commandContext) fetchApp(ctx context.Context, w io.Writer) (string, error) {
	...
	default:
		return "", fmt.Errorf("to start: fetch not supported on %s", runtime.GOOS)
	}
}
...
func (c *commandContext) fetchAppDarwin(ctx context.Context, w io.Writer) (string, error) {
	...
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("to start: %s not found in unpacked release at %s", appBundleName, staging)
	}
	return appPath, nil
}
...
func (c *commandContext) fetchAppWindows(ctx context.Context, w io.Writer) (string, error) {
	...
	if local == "" {
		return "", fmt.Errorf("to start: LOCALAPPDATA not set; cannot locate installed app")
	}
	appPath := windowsInstalledExe(local)
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("to start: installed app not found at %s", appPath)
	}
	return appPath, nil
}
...
func (c *commandContext) fetchAppLinux(ctx context.Context, w io.Writer) (string, error) {
	...
	if !isUsableBundle(appPath) {
		return "", fmt.Errorf("to start: AppImage not found at %s", appPath)
	}
	return appPath, nil
}
...
func requireAMD64() (string, error) {
	if runtime.GOARCH != "amd64" {
		return "", fmt.Errorf("to start: unsupported architecture %q on %s (only amd64 is published)", runtime.GOARCH, runtime.GOOS)
	}
	return "x64", nil
}
...
func assetArch(goarch string) (string, error) {
	...
	case "amd64":
		return "x64", nil
	default:
		return "", fmt.Errorf("to start: unsupported architecture %q", goarch)
	}
}
