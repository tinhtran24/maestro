...
func newProjectAddCommand(ctx *commandContext) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Register a local git repo as a project so sessions can be spawned in it.\n\n" +
			...
			"Maestro initializes/adopts the parent as the root repo and gitignores children.",
		Args: noArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if opts.path == "" {
...
func writeProjectList(cmd *cobra.Command, projects []projectSummary) error {
	...
	if len(projects) == 0 {
		...
		_, err := fmt.Fprintln(out, "Run `maestro project add --path <path>` to register one.")
		return err
	}
