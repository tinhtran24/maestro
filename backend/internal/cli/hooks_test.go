...
func TestHooks_NotificationReportsWaitingInput(t *testing.T) {
	t.Setenv("MAESTRO_SESSION_ID", "to-7")
	cfg := setConfigEnv(t)
...
func TestHooks_RejectsMalformedSessionID(t *testing.T) {
	t.Setenv("MAESTRO_SESSION_ID", "../etc/passwd")
	cfg := setConfigEnv(t)
...
func TestHooks_NoSessionIDIsNoOp(t *testing.T) {
	...
	if capture.hits != 0 {
		t.Errorf("expected no daemon call for a non-Maestro session, got %d", capture.hits)
	}
}
...
// $MAESTRO_DATA_DIR/hooks.log — and a delivered hook must not write the file at all.
func TestHooks_DeliveryFailureGoesToHooksLog(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			wantLog: true,
			wantIn:  []string{"maestro hooks claude-code session-end", "session=to-7"},
		},
		{
...
func TestHooks_HooksLogTruncatesPastCap(t *testing.T) {
	...
	if !strings.Contains(string(data), "maestro hooks claude-code session-end") {
...
func TestHooks_DaemonErrorIsSwallowed(t *testing.T) {
	...
	if !strings.Contains(errOut, "maestro hooks") {
