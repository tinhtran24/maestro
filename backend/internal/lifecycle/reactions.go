...
// ReviewResult is the already-persisted result of an Maestro-internal review pass.
...
func (m *Manager) ApplyReviewBatch(ctx context.Context, workerID domain.SessionID, batchID string, results []ReviewResult) (ReviewDeliveryOutcome, error) {
	...
	fmt.Fprintf(&msg, "[Maestro reviewer] Maestro's internal code reviewer submitted %d review(s) requesting changes.\n", len(results))
	var sigParts []string
...
// ApplyReviewResult reacts to a completed Maestro-internal review pass after the
...
//     for "assignee changed away from Maestro" is reserved for the write-side work
//     tracked by #40.
