...
import (
	...
	previewutil "github.com/tinhtran24/maestro/backend/internal/preview"
...
func (c *SessionsController) spawn(w http.ResponseWriter, r *http.Request) {
	...
	// and the read model falls back to the session id). `maestro spawn` makes it
	...
	if domain.IsNativeCLIModel(in.Harness, in.Model) {
		envelope.WriteAPIError(w, r, http.StatusBadRequest, "bad_request", "NATIVE_CLI_MODEL_NOT_ALLOWED", "native CLI models cannot be selected when creating a task", nil)
		return
	}
...
//
// Every call bumps the session's preview revision, so re-running `maestro preview`
...
// clearPreview resets a session's browser preview to empty (`maestro preview
...
// runtime/workspace are torn down. Used by `maestro spawn --claim-pr` to undo a
...
// (via `maestro hooks <agent> <event>`). It funnels through the single
