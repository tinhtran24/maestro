...
| `start` | Fetch (if needed) and open the Thanos desktop app | Launching the app | [commands/start.md](commands/start.md) |
| `stop` | Stop the Thanos daemon | Shutting down Thanos | [commands/stop.md](commands/stop.md) |
| `status` | Show daemon status | Verifying the daemon is up and healthy | [commands/status.md](commands/status.md) |
| `doctor` | Run local health checks | Diagnosing Thanos setup problems | [commands/doctor.md](commands/doctor.md) |
| `import` | Import projects from a legacy Thanos install | Migrating from the old flat-file store | [commands/import.md](commands/import.md) |
...
- Session and project ids are shown by `to session ls` and `to project ls`.
- `--agent` is an alias for `--harness` on `to spawn`.
