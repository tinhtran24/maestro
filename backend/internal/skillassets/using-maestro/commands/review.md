...
```
maestro review <subcommand> [args] [flags]
```
...
**Syntax:**
```
maestro review submit [worker-session-id] [flags]
```
...
maestro review submit mer-3 --run review-run-1 --verdict approved
```

```bash
# Submit a changes-requested review with a body from stdin
echo "Please fix the null check on line 42." | maestro review submit --session mer-3 --run review-run-1 --verdict changes_requested --body -
```
