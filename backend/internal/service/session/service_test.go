...
func TestSpawnEmitsFirstSessionOnboardingAndDuration(t *testing.T) {
	...
	if sink.events[0].Name != "to.session.spawned" || sink.events[1].Name != "to.onboarding.first_session_spawned" {
...
func TestSpawnFailedEmitsDuration(t *testing.T) {
	...
	if len(sink.events) != 1 || sink.events[0].Name != "to.session.spawn_failed" {
...
func TestSpawnEmitsTelemetryOnSuccess(t *testing.T) {
	...
	ev := ts.events[0]
	if ev.Name != "to.session.spawned" || ev.Source != "session_service" {
		t.Fatalf("event = %+v", ev)
	}
...
func TestSpawnEmitsTelemetryOnFailure(t *testing.T) {
	...
	ev := ts.events[0]
	if ev.Name != "to.session.spawn_failed" || ev.Source != "session_service" || ev.Level != ports.TelemetryLevelError {
		t.Fatalf("event = %+v", ev)
	}
...
func TestSpawnOrchestratorVerifiesReplacementHarness(t *testing.T) {
	...
	fc := &fakeCommander{
		spawnRecord: domain.SessionRecord{
			...
			Metadata:  domain.SessionMetadata{Branch: "to/mer-orchestrator"},
		},
	}
	svc := &Service{manager: fc, store: st}
